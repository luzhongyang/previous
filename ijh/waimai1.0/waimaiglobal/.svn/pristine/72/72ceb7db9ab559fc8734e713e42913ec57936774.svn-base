...
class Ctl_Biz_Tongji extends Ctl_Biz
{
    ...
    public function order($params)
    {
        ...
        $total_count = K::M('order/order')->count(array('shop_id'=>$this->shop_id,'order_status'=>8));
    ...
    {
        $tweek = date('Ymd', __TIME - 8 * 86400);
        $tmonth = date('Ymd', __TIME - 30*86400); 
        $filter = array();
        $filter['shop_id'] = $this->shop_id;
        //if($week_count = K::M('order/product')->count_by_product(array('shop_id'=>$this->shop_id, 'dateline')))       
    }
