...
class Ctl_Biz_Weixin_Reply extends Ctl_Biz_Weixin 
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'biz/weixin/reply/items.html';
    }
        ...
        if($data = $this->checksubmit('data')){
            if(!$data = $this->check_fields($data, 'title,intro,content,jumpurl')){
                ...
                if($reply_id = K::M('weixin/reply')->create($data)){
                    $this->msgbox->set_data('forward', $this->mklink('biz/weixin/reply:index'));
                    $this->msgbox->add('添加微信素材成功');
                }
            }
        }else{
            $this->tmpl = 'biz/weixin/reply/create.html';
        }
    }
    ...
    public function edit($reply_id=null)
    {
        if(!($reply_id = (int)$reply_id) && !($reply_id = (int)$this->GP('reply_id'))){
            ...
            $this->tmpl = 'biz/weixin/reply/edit.html';
        }
    }
    ...
    public function dialog($page=1)
    {
        ...
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'biz/weixin/reply/dialog.html';
    }
