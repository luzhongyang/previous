...
class Ctl_Biz_Weixin_Card extends Ctl_Biz_Weixin {
    ...
    public function index($page = 1) {
        ...
        if ($res['errcode'] == 0 && $res['total_num'] > 0) {
            ...
            $this->tmpl = 'biz/weixin/card/index.html';
        } else {
            $this->msgbox->add('您没有卡劵,请先在微信中添加', 213);
        }
    }
    ...
    public function get_card($id) {
        ...
        $qrurl = $this->mklink('biz/weixin/card:wxqrcode', array(), array('id' => $id));
    ...
    public function show() {
        ...
        $qrurl = $this->mklink('biz/weixin/card:wxqrcode2', array(), array('codeurl' => $url));
        header("Location:{$qrurl}");
    }
            ...
            if ($cart['errcode'] == 0) {
                if ($cart['card']['begin_time'] < time() && $cart['card']['end_time'] > time() && $cart['can_consume'] === true) {
                    ...
                    if ($res3['errcode'] == 0 || $res3['errmsg'] == 'ok') {
                        ...
                        $this->msgbox->add('核销失败', 214);
                    }
                } else {
                    $this->msgbox->add('该卡劵已过期 或已被核销', 213);
                }
            } else {
                $this->msgbox->add('该卡劵不存在 请查证卡劵ID', 211);
            }
        } else {
            $this->pagedata['id'] = $id;
            $this->tmpl = 'biz/weixin/card/form.html';
        }
    }
    ...
    public function wxqrcode2() {
        ...
        $this->tmpl = 'biz/weixin/card/wxqrcode2.html';
    }
    ...
    public function wxqrcode() {
        ...
        $this->tmpl = 'biz/weixin/card/wxqrcode.html';
    }
