...
class Ctl_Member extends Ctl
{
    ...
    public function updatemobile($params)
    {
        ...
        if(!$params['new_mobile']){
            $this->msgbox->add(L('新手机号不能为空'),233);
        }else if(!$new_mobile = K::M('verify/check')->mobile($params['new_mobile'])){
            $this->msgbox->add(L('新手机号不不正确'),223);
        }else if(!$params['sms_code']){
            $this->msgbox->add(L('短信验证码不能为空'),231);
    ...
    public function unbindweixin($params){
        $this->check_login();
        $data = array(
                'wx_openid' => '',
                'wx_unionid' => ''
        );
        $unbind = K::M('member/member')->update($this->uid,$data);
        if($unbind){
           $this->msgbox->add('解绑成功');
        }else{
           $this->msgbox->add(L('解定失败'),214);
        }
    }
