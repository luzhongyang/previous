...
class Ctl_Member_Collect extends Ctl
{
    ...
    public function items($params)
    {
        ...
        if($collect_list = K::M('shop/collect')->items($filter, null, $page, 10)){
             ...
             foreach($shop_list as $k=>$val){
                if($val['youhui']){
                    ...
                    foreach ($youhui as $kk => $v) {
                        $str .= sprintf(L('满%s减%s'), (int) $v['order_amount'], (int) $v['youhui_amount']) . ',';
                    }
            ...
            $items = array();
            foreach($shop_list as $k=>$val){
                $items[] = $this->filter_fields('shop_id,city_id,city_name,d,youhui,orders,cate_title,title,cate_id,phone,logo,lat,lng,addr,score,comments,praise_num,min_amount,first_amount,pei_amount,pei_type,yy_status,yy_stime,yy_ltime,is_new,online_pay,info,orders,youhui,freight', $val);
            }
        }else{
            $items = array();
        }
