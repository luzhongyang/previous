...
class Ctl_Order extends Ctl
{
    ...
    public function items($params)
    {
        ...
        if($order_list = K::M('order/order')->items($filter, array('order_id'=>'DESC'), $page, $limit, $count)){
            ...
            foreach($order_list as $k=>$val){            
                ...
                $items[$k]['jifen'] = (int)($jifen['jifen_ratio']*$val['amount']);
                if($shop_list[$val['shop_id']]){
                    $items[$k]['shop'] = array($shop_list[$val['shop_id']]);
                }else{
                    $items[$k]['shop'] = array(array('shop_id'=>'0', 'shop_title'=>'', 'photo'=>'', 'logo'=>'default/shop_logo.png'));
                }
    ...
    public function preinfo($params)
    {
        ...
        if(!$shop_id = (int)$params['shop_id']){
            ...
            $data = array('freight'=>$shop['freight'],'hongbao_id'=>$hongbao_id, 'hongbao_count'=>$hongbao_count, 'hongbao'=>$hongbao, 'addr_id'=>$addr_id);
            if($shop['first_amount'] && !$this->MEMBER['orders']){
                $data['first_youhui'] = $shop['first_amount'];
            }else{
                $data['first_youhui'] = '0';
            }
            $data['order_youhui'] = $shop['order_youhui'];
            if(CLIENT_OS == 'ANDROID'){ //android app 字段名与之前的重复导致类型错误
                $data['addr_detail'] = $addr;
            }else{
                $data['addr'] = $addr;
            }
    ...
    public function detail($params)
    {
        ...
        if(!$order_id = (int)$params['order_id']){
            ...
            if(!$staff = K::M('staff/staff')->detail($order['staff_id'])){
               $order['staff'] = array('staff_id'=>0, 'name'=>'', 'mobile'=>'','lat'=>'','lng'=>'');               
            }else{
