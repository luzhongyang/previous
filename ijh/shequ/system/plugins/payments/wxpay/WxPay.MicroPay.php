...
class MicroPay
{
	...
	public function pay($microPayInput)
	{
		...
		if(!array_key_exists("return_code", $result)
			|| (!array_key_exists("out_trade_no", $result) && $result['err_code'] != 'USERPAYING')
			|| !array_key_exists("result_code", $result))
		{
			//echo "接口调用失败,请确认是否输入是否有误！";

			K::M('system/logs')->log('Wxpay.MicroPay', array($result, $microPayInput));
			throw new WxPayException($result['err_code_des'].'['.$result['err_code'].']'.'+++micropay error');
		}else{
			K::M('system/logs')->log('Wxpay.MicroPay2', array($result, $microPayInput));
		}
