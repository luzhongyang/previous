...
{ 
    ...
    {      
        ...
        if($month = K::M('waimai/orderproduct')->count_sales($this->shop_id, $between)) {
            foreach($month as $k=>$v) {
                $pids[] = $v['product_id'];
            }
            if($cate = K::M('waimai/product')->items_by_ids($pids)) {
                foreach ($month as $k => $v) {
                    foreach($cate as $k1=>$v1) {
                        if($v['product_id'] == $v1['product_id']) {
                            $month[$k]['title'] = $v1['title'];
                        }
                    } 
                }
            }
        ...
        if($week = K::M('waimai/orderproduct')->count_sales($this->shop_id, $between)) {
            foreach($week as $k=>$v) {
                $pids[] = $v['product_id'];
            }
            if($cate = K::M('waimai/product')->items_by_ids($pids)) {
