...
class Ctl_Waimai_Waimai extends Ctl
{
    ...
    public function cate()
    {
        ...
        $this->tmpl = 'merchant:waimai/cate/index.html';
    }
    
    
    public function cate_create($parent_id=null)
    {
        if($data = $this->checksubmit('data')){
            ...
            if($cate_id = K::M('waimai/productcate')->create($data)){
                ...
                $this->msgbox->set_data('forward', $this->mklink('merchant/waimai/waimai:cate'));
            } 
        }else{
            ...
            $this->tmpl = 'merchant:waimai/cate/create.html';
        }
        
    }

    public function cate_edit($cate_id=null)
    {
        if(!($cate_id = (int)$cate_id) && !($cate_id = $this->GP('cate_id'))){
            ...
            if(K::M('waimai/productcate')->update($cate_id, $data)){
                ...
                $this->msgbox->set_data('forward',  $this->mklink('merchant/cate:waimai'));
            }  
        }else{
            $this->pagedata['cates'] = K::M('waimai/productcate')->items(array('parent_id'=>0, 'shop_id'=>$this->shop_id));
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'merchant:cate/waimai/edit.html';
        }    
    }

    public function cate_delete($cate_id=null)
    {
        if($cate_id = (int)$cate_id){
            if(!$detail = K::M('waimai/productcate')->detail($cate_id)){
                ...
                if($detail['parent_id'] == 0) {
                    $count = 0;
                    if($items = K::M('waimai/productcate')->items(array('parent_id'=>$detail['cate_id']),$orderby,$page,$limit,$counts)) {
                        foreach($items as $k=>$v) {
                            if(K::M('waimai/productcate')->delete($v['cate_id'])) {
                                $count += 1;
                            }
                        }
                    }
    ...
    public function product_create()
    {
        if($data = $this->checksubmit('data')){
            ...
            if($product_id = K::M('waimai/product')->create($data)){
                $this->msgbox->add('添加内容成功');
                $this->msgbox->set_data('forward',  $this->mklink('merchant/product:waimai'));
            } 
        }else{
