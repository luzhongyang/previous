...
class Ctl_Ucenter_Collect extends Ctl_Ucenter
{
    ...
    {   
        $this->tmpl = 'fenxiao/ucenter/collect/index.html';
    }
    ...
    public function loaditems($page=1)
    {
        ...
        $page = max((int)$page, 1);
        $limit = 10;
        if(!$items = K::M('fenxiao/collect')->items($filter,array('collect_id'=>'desc'),$page, $limit, $count)){
            $items = array();
        }else{
            $product_ids = array();
        ...
        $count_num = K::M('fenxiao/collect')->count($filter);
        if($count_num <= $limit){
            $loadst = 0; 
        }else{
            $loadst = 1; 
        ...
        $this->tmpl = 'fenxiao/ucenter/collect/loaditems.html';
