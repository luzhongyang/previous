...
class Ctl_Trade_Payment extends Ctl
{
    ...
    public function return_verify($code, $from = null)
    {
        ...
        $forward = $this->mklink('ucenter',null,null,'base');
        ...
        if(!($order_id = (int) $order_id) && !($order_id = (int) $this->GP('order_id'))){
            ...
            if($order['order_status'] < 0){
                ...
                if($code == 'money'){//使用余额支付
                    ...
                    if($url = K::M('trade/payment')->order($code, $order,null,$this->request)){

                        if(!defined('IN_WEIXIN') && ($code == 'wxpay')){
                            $qrurl = $this->mklink('trade/payment:wxqrcode', array(), array('codeurl' => $url, 'amount' => $order['amount'], 'trade_no' => $url['trade_no']),'base');
                            header("Location:{$qrurl}");
                        }else if(defined('IN_WEIXIN') && ($code == 'alipay')){
                            $this->pagedata['payurl'] = $url;
                            $this->tmpl = 'pchome/trade/payment/wxalipay.html';
                        }else{
                            header("Location:{$url}");
                        }
                    }
    ...
    public function url_auto_location($order,$order_id)
    {
        ...
        if($order['from'] == 'weidian'){
            ...
            if($detail['type'] == 'default'){
                ...
                if($order['from'] == 'mall'){
                    $link = $this->mklink('ucenter:mall',null, null, 'base');
                }else{
    ...
    public function money($code = null, $amount = null)
    {
        ...
        if(!$code){
            ...
            if($ret = K::M('trade/payment')->money($this->uid, $code, $amount)){
                ...
                if($code == 'wxpay'){
                    $qrurl = $this->mklink('trade/payment:wxqrcode', array(), array('codeurl' => $url, 'amount' => $amount, 'trade_no' => $trade_no),$this->request['url']);
                    header("Location:{$qrurl}");
                }else if($code == 'alipay'){
                    $this->pagedata['payurl'] = $url;
                    $this->tmpl = 'pchome/trade/payment/wxalipay.html';
    ...
    public function yzbill($code = null, $bill_id = null)
    {
        if(!($bill_id = (int) $bill_id) && !($bill_id = (int) $this->GP('bill_id'))){
            $this->msgbox->add('账单不存在', 210);
        }else if(empty($code) && !($code = $this->GP('pay_code'))){
            $this->msgbox->add('账单不存在', 211);
            ...
            $this->msgbox->add('账单已被支付，请勿重复提交', 216);
                ...
                if(!$trade = K::M('trade/payment')->yzbill('money', $bill)){
                    ...
                    if($log['payed']){
                        ...
                        if($res = K::M('trade/payment')->payed_yzbill($log, $trade)){
                            ...
                            $this->msgbox->add('订单支付成功');
                            $link = $this->mklink('xiaoqu/bill:detail',array($bill_id));
                            $this->msgbox->set_data('forward',$link);
                        }
                    }
                }
            }else{
                if($url = K::M('trade/payment')->yzbill($code, $bill)){
                    if(!defined('IN_WEIXIN') && ($code == 'wxpay')){
        ...
        if(!($order_id = (int)$order_id) && !($order_id = (int) $this->GP('order_id'))){
            ...
            if($code == 'money'){//使用余额支付
               if(!$trade = K::M('trade/payment')->cloud('money', $order)){
                    ...
                    if($log['payed']){
                        ...
                        if($res = K::M('trade/payment')->payed_cloud($log, $trade)){
                            if($this->MEMBER['wx_openid']){
                                ...
                                //K::M('weixin/wechat')->admin_wechat_client()->sendTempMsg($this->MEMBER['wx_openid'], $wx_config['bill_id'], $url, $a);
                            }
                            $this->msgbox->add('订单支付成功');
                ...
                if($url = K::M('trade/payment')->cloud($code, $order)){
                    if(!defined('IN_WEIXIN') && ($code == 'wxpay')){
    ...
    public function wxqrcode()
    {
        ...
        if(!$trade_no = $this->GP('trade_no')){
            exit('params error3');
        }
        $amount = sprintf("%.2f", $amount);
        $this->pagedata['codeurl'] = $codeurl;
        $this->pagedata['amount'] = $amount;
        $this->pagedata['trade_no'] = $trade_no;
        $this->tmpl = 'pchome/trade/payment/wxqrcode.html';
    }
