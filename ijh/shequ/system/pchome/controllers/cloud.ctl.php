...
class Ctl_Cloud extends Ctl
{
    ...
    public function index(){
       //即将揭晓
       $items['rest'] = K::M('cloud/attr')->items_by_rest(array('closed'=>0,'status'=>0),8);
       //人气商品 --- cloud_num
       $items['renqi'] = K::M('cloud/attr')->items(array('closed'=>0,'status'=>0),array('cloud_num'=>'desc'),1,8);
       //新品
       $items['new'] = K::M('cloud/attr')->items(array('closed'=>0,'status'=>0),array('attr_id'=>'desc'),1,12);
       //精品
       $items['jingpin'] = K::M('cloud/attr')->items(array('closed'=>0,'status'=>0,'is_fine'=>1),array(),1,8);
       //价格
       ...
       $this->pagedata['cloud_cates'] = $cates;
       $this->tmpl = 'pchome/cloud/index.html';
    }
    ...
    { //云购列表
        ...
        $this->tmpl = 'pchome/cloud/items.html';
    } 
       ...
       if(!$attr_id = (int) $attr_id){
           ...
           if($detail['status'] == 1){
               $this->lottery($attr_id);
                ...
                $this->tmpl = 'pchome/cloud/detail.html'; 
           }
       }
   }
   ...
   public function code($back_attr_id,$attr_id=null,$uid=null)
    ...
    public function share($goods_id,$page=1){
        $filter = $pager = array();
        $pager['page'] = max(intval($page), 1);
        $pager['limit'] = $limit = 5;
        ...
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'pchome/cloud/share.html';
    }
