...
class Ctl_Ucenter_Order extends Ctl_Ucenter
{
    ...
    public function index()
    {
        ...
        $this->tmpl = 'weidian/ucenter/order/index.html';
    }
    ...
    public function loaditems($page=1)
    {
        ...
        $this->tmpl = 'weidian/ucenter/order/loaditems.html';
    ...
    public function detail($order_id){
        if(!$order_id = (int)$order_id){
            ...
            $this->tmpl = 'weidian/ucenter/order/detail.html';
        }
    }
    ...
    public function cancel($order_id){ //订单取消
        if(!$order_id = (int)$order_id){
            ...
            if(K::M('weidian/order')->cancel($order_id,$order,$info)){
    ...
    public function comment($order_id){
        if(!$order_id){
            ...
            $this->pagedata['orderproducts'] = $orderproducts;
            $this->pagedata['order'] = $order;
            $this->tmpl = 'weidian/ucenter/order/comment.html';
        }
    }
    ...
    public function pay($order_id)
    {
        ...
        if($order['from'] == 'weidian'){
            $worder = K::M('weidian/order')->detail($order_id);
        ...
        if(!$order_id){
               ...
               if($order['from'] == 'weidian'){
                    if($detail = K::M('weidian/order')->detail($order_id)){
                        if($detail['type'] == 'default'){
                            ...
                            $this->msgbox->set_data("forward", $this->mklink('ucenter/pintuan',null,null,'base'));
                        }
                    }
                }
        }else{
            ...
            $this->pagedata['order'] = $order;
            $this->tmpl = 'weidian/ucenter/order/pay.html';
        }
    }
