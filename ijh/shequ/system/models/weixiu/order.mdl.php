...
class Mdl_Weixiu_Order extends Mdl_Table
{   
  
    protected $_table = 'weixiu_order';
    ...
    public function order_count($filter)
    {
        $where = '1';
        if(is_array($filter)){
            if(isset($filter['weixiu'])){
                $where = $this->where($filter['weixiu'], 'ext.');
