...
class Mdl_Member_WxLogin extends Model
{
    ...
    public function wxlogin_url($request)
    {
        $cfg = K::$system->config->get('connect');
        if(defined('IN_MOBILE')){
            ...
            $site = K::$system->config->get('site');
            $callback = K::M('helper/link')->mklink('passport:wxcallback', null, null, $request['url']);
        }
        if (empty($cfg['weixin_is_open'])) {
            $this->msgbox->add('很抱歉网站管理员还未开启微信登录功能', 201);
        }
        else {
            $session_state = md5(uniqid(rand(), TRUE));
            K::$system->session->start();
    ...
    public function wxcallback($code, $state)
    {
        ...
        if(defined('IN_MOBILE')){
            ...
            $site = K::$system->config->get('site');
            $callback = K::M('helper/link')->mklink('passport:wxcallback', array(), array(), 'www');
        }
        if (empty($cfg['weixin_is_open'])) {
            $this->msgbox->add('很抱歉网站管理员还未开启微信登录功能', 201);
            return false;
    ...
    public function login($info)
    {
        ...
        $wx_info = json_decode($str,true);  //微信用户基本信息
        if(empty($wx_info)){
            $this->msgbox->add('授权过期请重试', 201);
            return false;
        }
        $wx_cookie = array(
            ...
            'face'=>$wx_info['headimgurl']
        );
        $cookie = K::M('system/cookie')->get('wxlogin');

        if(!$cookie){
            K::M('system/cookie')->set('wxlogin',json_encode_ex($wx_cookie));
        }
        /*if($m = K::M('member/weixin')->detail_by_unionid($wx_info['unionid'])){
            K::M('member/auth')->manager($m['uid']);
            return true;            
        }*/
        if($m = K::M('member/member')->find(array('wx_unionid'=>$wx_info['unionid']))){  //如果该用户已绑定unionid则自动登录
            K::M('member/auth')->manager($m['uid']);
            return true;            
        }
        return false;

        /*if($uid = K::$system->uid){ //如果当前登录了帐号又去扫码
            if($m = K::M('member/member')->detail($uid)){
                K::M('member/member')->update($uid, array('wx_openid'=>$wx_info['openid'], 'wx_unionid'=>$wx_info['unionid']), true);
            }
        }else{
            echo '文件' . __FILE__ . '第' . __LINE__ . '行';
            die;
