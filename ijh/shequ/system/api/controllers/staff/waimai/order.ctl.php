...
class Ctl_Staff_Waimai_Order extends Ctl_Staff
{
    ...
    public function detail($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            if(!$shop = K::M('waimai/waimai')->detail($order['shop_id'])){
                $order['shop'] = array('shop_id'=>0, 'title'=>'', 'phone'=>'', 'logo'=>'default/waimai_shop.png', 'addr'=>'');
            }else{
                $shop = $this->filter_fields('shop_id,title,phone,logo,addr',$shop);
                $order['shop'] = $shop;
            }
            if($product_list = K::M('waimai/orderproduct')->items(array('order_id'=>$order_id), null, 1, 50)){
    ...
    public function qiang($params)
    {
        ...
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff', 'log'=>sprintf(L('骑手(%s:%s)准备为您配送'), $this->staff['name'],$this->staff['mobile']), 'status'=>'2');
    ...
    public function pei($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff', 'log'=>sprintf(L('骑手(%s:%s)开始为您配送'), $this->staff['name'],$this->staff['mobile']), 'status'=>'4');
            K::M('order/log')->create($log);
            //通知会员
            K::M('order/order')->send_member("骑手已取货", $log['log'], $order);
            $this->msgbox->set_data('data', array('order_id'=>$order_id));
        }
    }
    ...
    public function finshed($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff', 'log'=>sprintf(L('骑手(%s:%s)已经为您送达'), $this->staff['name'],$this->staff['mobile']), 'type'=>'4');
            K::M('order/log')->create($log);
            //通知会员
