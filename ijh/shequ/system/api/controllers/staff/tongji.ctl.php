...
class Ctl_Staff_Tongji extends Ctl
{
    ...
    public function order($params)
    {
        ...
        if(!$this->staff_id){
			...
			K::M('system/logs')->log('staff_tongji_sql',$this->system->db->SQLLOG());
    ...
    public function amount($params)
    {
        ...
        if(!$this->staff_id){
            ...
            $filter = array(
                'staff_id'=>$this->staff_id, 
                'dateline'=>$month_start.'~'.$yesterday, 
            );
            //最近30天订单量
            ...
            $today_start = strtotime(date("Ymd"));
            $today_end   = strtotime(date("Ymd")." 23:59:59");
            $filter = array('staff_id' => $this->staff_id, 'dateline'=>$today_end."~".$today_start);
