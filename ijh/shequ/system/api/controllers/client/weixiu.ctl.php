...
class Ctl_Client_Weixiu extends Ctl
{
    ...
    public function items($params)
    {
        ...
        if(!$_LNG || !$_LAT){
            ...
            //$filter = array('from'=>'weixiu', 'closed'=>0, 'audit'=>1, 'verify_name'=>1);
            $filter = array('from'=>'weixiu', 'closed'=>0, 'audit'=>1);
            // {{{####
            ...
            if($cate_id = $params['cate_id']){
                if($cate_ids = K::M('weixiu/cate')->children_ids($cate_id)){
            ...
            if(($page < 10) && ($staff_list = K::M('staff/staff')->staff_items($filter, $orderby, $_page, $_limit, $count))){
                ...
                if($attr_list = K::M('weixiu/attr')->items(array('staff_id'=>$staff_ids), null, 1, 100)){
    ...
    {  
        ...
        if(!$_LNG || !$_LAT){
            ...
            $unit = K::M('data/unit')->unit_list();
            $attr = K::M('weixiu/attr')->items(array('staff_id'=>$detail['staff_id']));
            foreach($attr as $k => $v){
                $attr[$k] = K::M('weixiu/cate')->detail($v['cate_id']);
            ...
            $detail['juli'] = K::M('helper/round')->juli($detail['lng'], $detail['lat'], $_LAT, $_LNG);  //距离
            $detail['juli_label'] = K::M('helper/format')->juli($detail['juli']);  //距离
