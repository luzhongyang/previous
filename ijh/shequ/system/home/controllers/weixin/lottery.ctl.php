...
class Ctl_Weixin_Lottery extends Ctl_Weixin
{
	...
	public function show($lottery_id)
	{
		if(!($lottery_id = (int)$lottery_id) && !($lottery_id = $this->GP('lottery_id'))){
			...
			if($record = K::M('weixin/lotterysn')->items($filter, null, $page, $limit, $count)){
	...
	{	
		...
		$Lottery= K::M('weixin/lottery')->detail($id);
	...
	{	
		...
		if($record 	= K::M('weixin/lotterysn')->items(array('openid'=>$openid),null,null,null,$count)){
			...
			if ($Lottery['ltime'] < $now) { //过期
				...
				//最多抽奖次数
				$LotteryedRecordWhere=array('wx_id'=>$wx_id,'lottery_id'=>$id,'islottery'=>1);
				...
				if (0){
					...
					if ($record['usenums'] >= $Lottery['max_num'] ) {
						...
						if ($thisDayNums>=$Lottery['daynums']){
							...
							//
							$data_sn = array('uid'=>$member['uid'],'openid'=>$openid,'wx_id'=>$wx_id,'lottery_id'=>$Lottery['id'],'nickname'=>$wx_info['nickname'],'islottery'=>$isLottery,'sn'=>$data['sncode'],'prize'=>intval($data['prizetype']));
		...
		//}
		//
		$record 	=  K::M('weixin/lotterysn')->detail($sn_id);
