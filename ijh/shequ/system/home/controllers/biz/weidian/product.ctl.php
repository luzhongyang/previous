...
class Ctl_Biz_Weidian_Product extends Ctl_Biz_Weidian
{
    ...
    public function index($page=1)
    {
        ...
        $fenixao = K::M('fenxiao/fenxiao')->find(array('shop_id'=>$this->shop_id));
        $this->pagedata['shop'] = K::M('fenxiao/fenxiao')->detail($fenixao['sid']);
        $this->tmpl = 'biz/weidian/product/index.html';
    }
    ...
    public function set_is_fenxiao($product_id){
        ...
        }elseif($product['price_level_1']==0&&$product['is_fenxiao']==0){
            $this->msgbox->add('正在跳转');
            $this->msgbox->set_data("forward", $this->mklink('biz/weidian/product:edit',array($product_id)));
        }else{
    ...
    public function create()
    {
        if($data = $this->checksubmit('data')){
            ...
            if($product_id = K::M('weidian/product')->create($data)){           
                ...
                $this->msgbox->set_data('forward',  $this->mklink('biz/weidian:product'));
            }
        }else{
           ...
           $this->tmpl = 'biz/weidian/product/create.html';
        }   
    }
    ...
    public function edit($product_id=null)
    {
        if(!($product_id = (int)$product_id) && !($product_id = $this->GP('product_id'))){
            ...
            $this->pagedata['fenxiao'] = $fenxiao;
            $this->pagedata['shop'] = $this->shop;
            $this->tmpl = 'biz/weidian/product/edit.html';
        }       
    }
    ...
    public function check_price(){
        ...
        $data = $this->GP('data');
        if($data['is_fenxiao'] == 1&& !$data['price_level_1']){
            $this->msgbox->add('分销价格不能为空',211)->response();
        }
        $total_amount = $data['price_level_1'] + $data['price_level_2'] + $data['price_level_3'];
        if($attr_stock){
            $i = 0;
            foreach($attr_stock as $k=>$v){
                if($v['wei_price'] <= $total_amount){
                    $i++;
                }
            }
        }
        if($i>0){
            $this->msgbox->add('分销价格不正确',233)->response();
        }
    }
    ...
    public function set_price($product_id){
        ...
        $this->tmpl = 'biz/weidian/product/set_price.html';
    }
