...
class Ctl_Biz_Weidian_Pintuangroup extends Ctl_Biz_Weidian
{
    ...
    public function index($page)
    {
        if($this->shop['pintuan'] == 1){
           ...
           // $this->msgbox->set_data('forward', $this->mklink('biz/shop/opened'));
        }
        else{
            $filter = $pager = array();
            ...
            if($items = K::M('weidian/pintuan/group')->items($filter, array('group_id' => 'desc'), $page, $limit, $count)){
                ...
                foreach($items as $k => $v){
                    ...
                    $v['money_pre'] = $arr_product_title[$v['product_id']]['money_pre'];
            ...
            $this->tmpl = 'biz/weidian/group/index.html';
        }
    }
    ...
    public function status_ok($group_id)
    {
        ...
        //防止误操作,退多次
        if($arr_group['end_time']>time()){
            $return['message'] = "更新失败, 团编号: $group_id 为阶梯团, <br />成团结束时间 ".date('Y-m-d H:i:s',$arr_group['end_time'])." 后才能接单!";
        }
        else if(1 == $arr_group['status']){
