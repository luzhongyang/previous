...
class Ctl_Biz_Weixin_Keyword extends Ctl_Biz_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'biz/weixin/keyword/items.html';    
    }
        ...
        if($data = $this->checksubmit('data')){
            if(!$data = $this->check_fields($data, 'keyword,reply_id,content')){
                ...
                if($res = K::M('weixin/keyword')->find(array('shop_id'=>$this->shop_id,'keyword'=>$data['keyword']))){
                    ...
                    $this->msgbox->set_data('forward', $this->mklink('biz/weixin/keyword:index'));
                    $this->msgbox->add('添加关键字成功');
                }
            }
        }else{
            $this->tmpl = 'biz/weixin/keyword/create.html';
        }
    }
    ...
    public function edit($kw_id=null)
    {
        if(!($kw_id = (int)$kw_id) && !($kw_id = (int)$this->GP('kw_id'))){
            ...
            $this->tmpl = 'biz/weixin/keyword/edit.html';
        }
    }
