...
class Ctl_Biz_Weixin_Coupon extends Ctl_Biz_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'biz/weixin/coupon/index.html';
    }
		...
		if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'shop_id'=>$this->shop_id))){
				if($coupon_id = K::M('weixin/coupon')->create($data)){
					...
					$this->msgbox->set_data('forward', $this->mklink('biz/weixin/coupon/index'));
				} 
			}else{
           ...
           $this->tmpl = 'biz/weixin/coupon/create.html';
        }
	}
	...
	public function edit($coupon_id=null)
    {
        if(!($coupon_id = (int)$coupon_id) && !($coupon_id = $this->GP('coupon_id'))){
        	...
        	$this->tmpl = 'biz/weixin/coupon/edit.html';
        }
	}
	...
	public function sign($coupon_id=null)
	{
		if(!($coupon_id = (int)$coupon_id) && !($coupon_id = $this->GP('coupon_id'))){
			...
			if($error){
				$this->tmpl = 'biz/weixin/coupon/over.html';
			}
		}
	}
	...
	public function sn($coupon_id,$page_id)
	{
		if(!($coupon_id = (int)$coupon_id) && !($coupon_id = $this->GP('coupon_id'))){
			...
			$this->tmpl = 'biz/weixin/coupon/sn.html';
		}
	} 
	...
	public function show($sn)
	{
		if(!($sn = (int)$sn) && !($sn = $this->GP('sn'))){
			...
			$this->pagedata['detail'] = $detail;
			$this->tmpl = 'biz/weixin/coupon/show.html';
		}
	}
