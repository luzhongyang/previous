...
class Ctl_House extends Ctl
{
    ...
    public function items($cate_id){
        ...
        $cate_list = K::M('house/cate')->select();
        ...
        if($lng && $lat){
            ...
            $attrs = K::M('house/attr')->items(array('staff_id'=>$staff_ids));
            foreach($items as $k=>$v){
    ...
    public function staff_detail($staff_id){  //服务人员详情
        if(!$staff_id){
            ...
            $attr = K::M('house/attr')->items(array('staff_id'=>$detail['staff_id'],null,500));
            foreach($attr as $k => $v){
                $attr[$k]['cate'] = K::M('house/cate')->detail($v['cate_id']);
            }
            ...
            $this->tmpl = 'house/staff_detail.html';
        }
    }
