...
class Ctl_Waimai_Order extends Ctl
{
    ...
    public function order($shop_id)
    {
        ...
        if($zhuohao_id = $this->cookie->get('zhuohao_id')){
            $zhuohao = array();
            if ($zhuohao_id == 2147483647) { //通用桌号
                $zhuohao['title'] = '通用';
                $package_price = 0;
            }else{
                if (($zhuohao = K::M('yuyue/zhuohao')->detail($zhuohao_id)) && $zhuohao['shop_id'] == $shop_id) {
            ...
            if(!$addr_id || !($addr = K::M('member/addr')->detail($addr_id))){
                ...
                //计算出对应的配送费
                    if(!$shop['freight_stage']){
                        $this->msgbox->add('您的地址不在配送范围', 216)->response();
                    }else{
        ...
        if(IS_AJAX){
            if($params = $this->checksubmit('params')){
                ...
                if(!$shop_id = (int) $params['shop_id']){
                    ...
                    if(empty($shop_detail['yy_status']) || ($cur_time < $yy_stime || $cur_time > $yy_ltime)){
                    ...
                    }else if($params['pei_type'] != 3 && $params['pei_type'] != 4 && !($addr_id = (int)$params['addr_id'])){
                        $this->msgbox->add('请选择地址',227);
                    }else if($params['pei_type'] != 3 && $params['pei_type'] != 4 && !($addr_detail = K::M('member/addr')->detail($addr_id))){
                        $this->msgbox->add('地址不存在',228);
                    }else if($params['pei_type'] != 3 && $params['pei_type'] != 4 && K::M('helper/round')->juli($addr_detail['lng'], $addr_detail['lat'], $shop_detail['lng'], $shop_detail['lat']) > ($shop_detail['pei_distance']*1000)){
                       $this->msgbox->add('超出配送范围',229);
                        ...
                        if ($params['pei_type'] == 4 && !empty($zhuohao_id)) {// 如果是堂食，清空打包费
                            $package_price = 0;
                        }
                        ...
                        if($params['pei_type'] == 3 || ($params['pei_type'] == 4 && !empty($zhuohao_id)) ){
                            ...
                            if ($params['pei_type'] == 3) {
                                $data_order['pei_type'] = 3;// 自提
                            }elseif ($params['pei_type'] == 4) {
                                $data_order['pei_type'] = 4;// 堂食
                                $data_order['zhuohao_id'] = $zhuohao_id;
                            }
                        ...
                        if($order_id = K::M('order/order')->create($data_order)) {
                            ...
                            if($params['online_pay']==0 && ($order['pei_type']==3 || $order['pei_type'] == 4)) { 
                                //如果是自提单或者堂食，且选择了到付，直接生成消费码
                                K::M('waimai/order')->create_number($order_id);
                            }else if($order['online_pay']==1 && $order['pay_status']==0 && ($order['pei_type']==3 || $order['pei_type'] == 4)) { 
                                //如果自提单或者堂食单选择了在线支付且未支付，支付成功之后生成消费码
                            ...
                            if($zhuohao_id = $this->cookie->get('zhuohao_id')){// 清空桌号信息
