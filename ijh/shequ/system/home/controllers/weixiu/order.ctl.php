...
class Ctl_Weixiu_order extends Ctl
{
    ...
    public function index($cate_id){
        ...
        }else if(!$cate = K::M('weixiu/cate') -> detail($cate_id)){
            $this->msgbox->add('错误',211);
        }else{
        ...
        // }
           ...
           $data['uid'] = $this->uid;
            ...
            if($add = K::M('order/order')->create($data)){
                ...
                $data2['order_id'] = $add;
                if($add2 = K::M('weixiu/order')->create($data2)){
                     // 写入订单日志
    ...
    public function comment($order_id){  //评价
        if(!$order = K::M('order/order')->detail($order_id)){
            ...
            $this->msgbox->add('该订单不是维修订单',213);
        }else{
            $this->pagedata['order']= $order;
        }
        $this->tmpl = 'weixiu/comment.html';
    }
