...
class Ctl_Waimai_Product extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'admin:waimai/product/items.html';
    }
    public function so()
    {
        $this->tmpl = 'admin:waimai/product/so.html';
    }
    public function detail($product_id = null)
    {
        if(!$product_id = (int)$product_id){
            ...
            $this->tmpl = 'admin:waimai/product/detail.html';
        }
    }
    ...
    public function shop($shop_id=null)
    {
        if(!$shop_id = (int)$shop_id){
            ...
            $cates = K::M('waimai/productcate')->items(array('shop_id'=>$shop_id));
            $this->pagedata['cates'] = $cates;
            $this->tmpl = 'admin:waimai/product/shop.html';
        } 
    }
        ...
        if(!($shop_id = (int)$shop_id) && !($shop_id = $this->GP('shop_id'))){
            ...
            if($data = $this->checksubmit('data')){
                ...
                if($product_id = K::M('waimai/product')->create($data)){
                    ...
                    $this->msgbox->set_data('forward',  $this->mklink('waimai/product:shop',array($shop_id)));
                   
                } 
            }else{
                $this->pagedata['shop_id'] = $shop_id;
                $this->pagedata['shop'] = $shop;
                $this->tmpl = 'admin:waimai/product/create.html';
            }
        }
    }
    public function edit($product_id=null)
    {
        if(!($product_id = (int)$product_id) && !($product_id = $this->GP('product_id'))){
            ...
            if(K::M('waimai/product')->update($product_id, $data)){
                ...
                $this->pagedata['shop'] = K::M('waimai/waimai')->detail($detail['shop_id']);
                $this->pagedata['detail'] = $detail;
                $this->tmpl = 'admin:waimai/product/edit.html';
        }
    }
