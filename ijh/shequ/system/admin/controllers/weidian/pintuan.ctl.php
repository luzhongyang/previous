...
class Ctl_Weidian_Pintuan extends Ctl
{
    public function index($page = 1)
    {
        ...
        $this->tmpl = 'admin:weidian/pintuan/items.html';
    }
    public function so()
    {
        $this->tmpl = 'admin:weidian/pintuan/so.html';
    }
    public function detail($product_id = null)
    {
        if(!$product_id = (int) $product_id){
            ...
            $detail['detail'] = K::M('weidian/pintuan/product')->detail($detail['product_id']);
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'admin:weidian/pintuan/detail.html';
        }
    }
...
//        $error .= 'test error';
    ...
    public function edit($product_id = null)
    {
        if(!($product_id = (int) $product_id) && !($product_id = $this->GP('product_id'))){
        ...
        }else if(!$detail = K::M('weidian/pintuan/product')->detail($product_id, true)){
            ...
            //插入数据校验
            $this->validate_insert();
            if(K::M('weidian/pintuan/product')->update($product_id, $data)){
                /*
                 * 拼团价格格修改,
                 * 如果是阶梯团, 将原有拼团价格格,最低的那个,修改为拼团价格
                 * 查询每个楼层,然后存储
                ...
                if(1 == $_POST['data']['tuan_type']){
                    ...
                    foreach($_POST['data']['muti_num'] as $k => $v){
                        //将第一个价格,更新到产品表
                        if(1 == $k){
                            K::M('weidian/pintuan/product')->update($product_id, array('tuan_price' => $_POST['data']['muti_price'][$k], 'user_num' => $_POST['data']['muti_num'][$k]));
                        }
                        $insert_leve = array(
            ...
            if(1 == $detail['tuan_type']){
                $arr_level = K::M('weidian/pintuan/productlevel')->select(array('pintuan_product_id' => $detail['pintuan_product_id']), 'level');
