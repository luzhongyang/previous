...
//跑腿订单, 送达之后, 24小时, 自动结账
$filter = array('pay_status'=>1, 'order_status'=>array(4, 5), 'lasttime'=>'<:'.(__TIME-172800));
$filter[':SQL'] = "(`from` IN('house','weixiu' ,'paotui'))";
if($items = K::M('order/order')->items($filter, null, 1, 30, $confirm_paotui_count)){
    foreach($items as $k=>$v){
        K::M('order/order')->confirm($v['order_id'], $v, 'system');
//        echo $v['order_id'].' / ';
    }
...
echo "家政维修跑腿自动完成:{$confirm_paotui_count}\n";

echo "+----------------------------------------------------------------------------------------+\n";
K::M('system/logs')->log('__crontab_cron', array('one time'));