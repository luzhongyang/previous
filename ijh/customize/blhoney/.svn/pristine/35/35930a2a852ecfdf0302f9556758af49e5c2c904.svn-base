...
class Ctl_Order extends Ctl
{
    ...
    public function preinfo($params)
    {
        ...
        if(!$shop_id = (int)$params['shop_id']){
            ...
            $data['order_youhui'][] = array("order_amount"=>"0","youhui_amount"=>"0");
            foreach($shop['order_youhui'] as $k=>$v) {
                if($v['order_amount'] <= $first_price) {
                    $data['order_youhui'][$k] = $v; 
                }
            }
            
            // 根据order_amount对满减进行排序
            foreach ($data['order_youhui'] as $key=>$value){
                $order_amount[$key] = $value['order_amount'];
                $youhui_amount[$key] = $value['youhui_amount'];
            }
             
            array_multisort($order_amount,SORT_NUMERIC,SORT_DESC,$youhui_amount,SORT_STRING,SORT_ASC,$data['order_youhui']);
            // 红包
