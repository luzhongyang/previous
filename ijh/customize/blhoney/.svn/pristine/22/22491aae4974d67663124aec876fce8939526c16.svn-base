...
/*+=======================================
 + 美业预约服务JS购物车
 +=======================================*/
window.cart_list = window.cart_list || {};
window.beauty = {
    init: function () {
        cart_list = JSON.parse(Cookie.get("beauty")) || {};
        var json = window.cookies.get('beauty') || {};
    },
    addcart: function (shop_id, data, count) {
        cart_list[shop_id] = cart_list[shop_id] || {};
        if (typeof (cart_list[shop_id][data['product_id']]) == 'undefined') {
            cart_list[shop_id][data['product_id']] = data;
        }else {
            data['num'] = count;
            cart_list[shop_id][data['product_id']] = data;
        }
        Cookie.set('beauty', JSON.stringify(cart_list));
    },
    getcart: function () {
        return window.cart_list;
    },
    count: function (shop_id) {
        var count = 0;
        if (typeof (cart_list[shop_id]) != 'undefined') {
            for (var pid in cart_list[shop_id]) {
                count += parseInt(cart_list[shop_id][pid]['num'], 10);
            }
        }
        return count;
    },
    ...
    totalprice: function (shop_id) {
        ...
        var goods = cart_list[shop_id] || {};
        if(goods != undefined) {
            for (var pid in goods['undefined']) {
                if(goods['undefined'][pid]['price'] != undefined) {
                    total_price += parseFloat(goods['undefined'][pid]['price'], 10);
                }  
            }
        }
        return total_price;
    ...
    removeby: function (shop_id) {
        var obj = {};
        for (var sid in cart_list) {
            if (sid != shop_id) {
                obj[sid] = cart_list[sid];
            }
        }
        window.cart_list = {};
        Cookie.set('beauty', JSON.stringify(obj));
    }
}
...
/*+=======================================
 + 美业商城订单JS购物车
 +=======================================*/
window.mall_list = window.mall_list || {};
window.mall = {
    init: function () {
        mall_list = JSON.parse(Cookie.get("mall")) || {};
        var json = window.cookies.get('mall') || {};
    },
    addcart: function (data) {
        mall_list = mall_list || {};
        data['num'] = data['num'] || 1;
        if (typeof (mall_list[data['product_id']]) == 'undefined') {
            mall_list[data['product_id']] = data;
        } else if (mall_list[data['product_id']]['num'] >= data['sku']) {
            layer.open({content: "商品库存不足",time: 2});
        } else {
            var num = mall_list[data['product_id']]['num'] || 0;
            data['num'] += parseInt(num, 10);
            mall_list[data['product_id']] = data;
        }
        Cookie.set('mall', JSON.stringify(mall_list));
    },
    getcart: function () {
        return window.mall_list;
    },
    dec: function (product_id, num) {
        num = num || 1;
        mall_list = mall_list || {};
        var product_list = {};
        if (typeof (mall_list[product_id]) == 'undefined') {
            return true;
        } else {
            if(num == 'null') {
                for(var pid in mall_list) {
                    if (pid != product_id) {
                        product_list[pid] = mall_list[pid];
                    }
                }
            }else {
                mall_list[product_id]['num'] -= parseInt(num, 10);
                for (var k in mall_list) {
                    if (mall_list[k]['num'] > 0) {
                        product_list[k] = mall_list[k];
                    }
                }
            }  
        }
        
        mall_list = product_list;
        Cookie.set('mall', JSON.stringify(mall_list));
    },
    count: function () {
        var count = 0;
        if (typeof (mall_list) != 'undefined') {
            for (var pid in mall_list) {
                count += parseInt(mall_list[pid]['num'], 10);
            }
        }
        return count;
    },
    ...
    removeby: function () {
        var obj = {};
        window.mall_list = {};
        Cookie.set('mall', JSON.stringify(obj));
    },
}
...
function select_yuyue_time(week_array) { 
    //time获取当前小时数
    var st = '<div class="date_mask">';
    st +='<div class="cont">';
        st +='<div class="tabBox" id="weekSelect">';
        st +='<div class="week_box hd">';
        st +='<span class="prev"></span>';
        st +='<ul class="week_list"></ul>';
        st +='<span class="next"></span>';
        st +='</div>';
        st +='<div class="week_time_box">';
        st +='<div class="bd">';
        st +='</div>';
        st +='</div></div></div></div>';
    ...
    for(var i=1;i<=7;i++){
        if(i==1){
           html+='<div class="week_time" id="wt'+i+'" style="display:block;"><ul class="list">'; 
        }else{
            html+='<div class="week_time" id="wt'+i+'"><ul class="list">';
        }
        
        for(var ii=9;ii<=20;ii++){
            html+='<li hour="'+ii+'">'+ii+':00</li>';
        }
        html+='</ul></div>';
    }
 
    var week='';
    $.each(week_array, function(key, value){
         week+='<li val="'+(parseInt(key)+parseInt(1))+'" id="wl'+(parseInt(key)+parseInt(1))+'"><p>'+value.w+'</p><p class="graycl" style="font-size:10px;">'+value.d+'</p></li>';
    })
    ...
    $('.bd').html(html);
    $('#wl1').addClass('on');
    $('#service_time').attr('week',$('#wl1 .graycl').text());//设置默认选中周一
    
    
    $('.week_list li').click(function(){
        $('.week_list li').removeClass('on');
        $(this).addClass('on');
        var val = $(this).attr('val');
        $('.week_time').hide();
        $('#wt'+val).show();
        $('#service_time').attr('week',$(this).find('.graycl').text());
    })
    $('.bd li').click(function(){

        if($('#service_time').attr('week') == ''){
            alert('没有选择日期！');
            return false;
        }
        ...
        var week = $('#service_time').attr('week');
        $('#service_time').attr('hour',hour);
        $('#service_time').text(week+' '+hour+':00');
        $("input[name='params[ordered_time]']").val(week+' '+hour+':00:00');
        $(".date_mask").remove();
    ...
    $('.prev').click(function(){
        ...
        if(prev >2){
            prev = 1;
             ...
             $('#wl'+prev).hide().removeClass('on');
             $('#wl'+(parseInt(prev)+parseInt(1))).addClass('on');
             
             $('#wt'+prev).hide();
             $('.week_time').hide();
             $('#wt'+(parseInt(prev)+parseInt(1))).show();
    })
