...
{   
    ...
    protected $_cols = 'article_id,city_id,cat_id,from,page,title,thumb,desc,linkurl,views,favorites,allow_comment,comments,photos,hidden,is_banner,orderby,audit,closed,dateline';
    ...
    protected function _format_row($row)
    {
        ...
        if($row['linkurl']){
			...
			*/
			 if(empty($row['city_id'])){
                $row['link'] = $mdllink->mklink('hotstyle:detail', array($row['article_id']), array(), 'www');
            }else{
                $row['link'] = $mdllink->mklink('hotstyle:detail', array($row['article_id']), array(), $row['city_id']);
            }
        }        
        return $row;
    }
