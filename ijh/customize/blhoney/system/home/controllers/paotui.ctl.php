...
class Ctl_Paotui extends Ctl {
    ...
    public function songsubmit(){
        $this->check_login();
        $data = $this->GP('data');
        if(!$o_time = $data['q_time']){
            $this->msgbox->add('没有取件时间',211);
        }else if(!$time = $data['s_time']){
            $this->msgbox->add('没有送件时间',212);
        }else if(!$o_addr = $data['q_addr']){
            $this->msgbox->add('没有取件地',213);
        }else if(!$o_house = $data['q_num']){
            $this->msgbox->add('没有取件门牌号',214);
        }else if(!$o_contact = $data['q_name']){
            $this->msgbox->add('没有取件联系人',215);
        }else if(!$o_mobile = $data['q_phone']){
            $this->msgbox->add('没有取件联系人手机号',216);
        }else if(!$o_lng = $data['q_lng']){
            $this->msgbox->add('没有取件地址经度',217);
        }else if(!$o_lat = $data['q_lat']){
            $this->msgbox->add('没有取件地址纬度',218);
        }else if(!$addr = $data['s_addr']){
            $this->msgbox->add('没有送件地址',219);
        }else if(!$house = $data['s_num']){
            $this->msgbox->add('没有送件门牌号',220);
        }else if(!$contact = $data['s_name']){
            $this->msgbox->add('没有送件联系人',221);
        }else if(!$mobile = $data['s_phone']){
            $this->msgbox->add('没有送件联系人手机号码',222);
        }else if(!$lng = $data['s_lng']){
            $this->msgbox->add('没有送件地址经度',223);
        }else if(!$lat = $data['s_lat']){
            $this->msgbox->add('没有送件地址纬度',224);
        }else if(!$intro = $data['intro']){
            $this->msgbox->add('没有要求和描述',225);
        }/*else if(!$voice = $data['voice']){
            $this->msgbox->add('没有语音',227);
        }*/else if(!$amount = $data['amount']){
            $this->msgbox->add('请输入物件重量',228);
        }else{
            ...
            if($paotui_id = K::M('paotui/paotui')->create($datas)){
                K::M('paotui/log')->create(array('paotui_id'=>$paotui_id, 'log'=>'订单已提交', 'from'=>'member', 'type'=>1));
                $this->msgbox->add('订单已提交');
                $this->msgbox->set_data('data', array('paotui_id' => $paotui_id));
            }else{
                $this->msgbox->add('下单失败',229);
            }
        }   
    }
    ...
    public function uploadVoice(){
        ...
        if($mid){
            if($voice = K::M('weixin/wechat')->wechat_client()->download($mid)){
                file_put_contents(__IMG_DIR.'aaa.mp3', $voice);
    ...
    public function buy_handle(){
        ...
        if(!$intro = $data['intro']){
            $this->msgbox->add('没有填写要求和描述',225);
        }else if(!$photo = $file['photo1']){
            $this->msgbox->add('没有图片',226);
        }else if(!$addr_id = $data['addr_id']){
            $this->msgbox->add('没有选择收货地址',228);
        }else if(!$time = $data['time']){
            $this->msgbox->add('没有选择收货时间',228);
        }else if(!$paotui_amount = $data['paotui_amount']){
            $this->msgbox->add('没有跑腿费用',228);
        }else{
            $addr = K::M('member/addr')->find($addr_id);
            if($data['xiaofei']){
                $paotui_amount = $paotui_amount + $data['xiaofei'];
            }
            ...
            if($paotui_id = K::M('paotui/paotui')->create($datas)){
                ...
                $this->msgbox->add('下单失败'.print_r($this->system->db->SQLLOG()),229);
            }
