    ...
    public function return_verify()
    {
        if(defined('IN_MOBILE')){
            if(empty($_GET)) {//判断GET来的数组是否为空
                ...
                $log = "IN_MOBILE\n\nreturn_url_log:sign={$_GET[sign]}&verify_sign={$verify_sign_str}&".$this->_build_query($notify);
                $this->_logs($log);
                if ($verify_sign){
    ...
    public function notify_verify()
    {
        if(!defined('IN_APP') && defined('IN_MOBILE')){   
            if(empty($_POST)) {//判断GET来的数组是否为空
                ...
                $verify_sign_str = (string)$verify_sign;
                $log = "IN_MOBILE\n\nnotify_url_log:sign={$_POST[sign]}&verify_sign={$verify_sign_str}&".$this->_build_query($notify);
                $this->_logs($log);
                if ($verify_sign){                  
