...
class Ctl_Index extends Ctl
{
    ...
    {
        $text = $this->GP('text');
        $apikey = "AIzaSyCKSVxRbXbvxd7GXhV4eXqwTByZFmLi6C4";
        $url = 'https://maps.googleapis.com/maps/api/place/autocomplete/json?input='.$text.'&key='.$apikey.'&language=it&components=country:it';
        $json = json_decode(file_get_contents($url), true);  
        if(!$json['predictions']) {
            $this->msgbox->add('failed',210);
        }else {
            $this->msgbox->add('success');
            $this->msgbox->set_data('data', $json['predictions']);
        }  
    }
    ...
    public function detailByid()
    {
        $url = $json = array();
        $apikey = "AIzaSyCKSVxRbXbvxd7GXhV4eXqwTByZFmLi6C4";
        $ids = $this->GP('place_id');
        $len = count($this->GP('place_id'));

        for ($i = 0; $i < $len; $i++) {
            $url[$i] = 'https://maps.googleapis.com/maps/api/place/details/json?placeid='.$ids[$i].'&key='.$apikey;
            $json[$i] = json_decode(file_get_contents($url[$i]), true);  
        }
        
        if(!$json) {
            $this->msgbox->add('failed',211);
        }else {
            $this->msgbox->add('success');
            $this->msgbox->set_data('data', $json);
        }  
