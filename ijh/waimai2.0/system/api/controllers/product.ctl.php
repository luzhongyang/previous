...
class Ctl_Product extends Ctl
{
           ...
           $this->msgbox->add(L('商家不存在'),211);
            ...
            if($items = K::M('product/product')->items($filter, null, $page, $limit, $count)){
                foreach($items as $k=>$v){
                    $items[$k] = $this->filter_fields('product_id,cate_id,shop_id,title,photo,price,package_price,sales,stock,sale_type,sale_sku,sale_count,intro,dateline', $v);
    ...
    public function detail($params){
        ...
        if(!$product_id = $params['product_id']){
            $this->msgbox->add(L('错误的商品！'),210);
        }else if(!$detail = K::M('waimai/product')->detail($product_id)){
            $this->msgbox->add(L('错误的商品！'),211);
        }else{
            $detail['spec'] = array();
            if($detail['is_spec']){
                $spec = K::M('waimai/productspec')->items(array('product_id'=>$product_id),null,1,100);
            ...
            $share = array(
                'share_url'=>$this->mklink('shop/product_detail', array($detail['product_id']), null, 'www'),
