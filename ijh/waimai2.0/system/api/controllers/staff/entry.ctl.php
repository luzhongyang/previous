...
class Ctl_Staff_Entry extends Ctl
{
    ...
    {
        //多的字段  from   tixian_money  score  comments   attr  sex  age  intro updatetime
        //少的字段  账号密码,相关信息,
        $session = K::M('system/session')->start();
        $params['from'] = 'paotui'; //默认所有的都是跑腿,之前社区 waimai是归属paotui的
        if(!$city_id = $params['city_id']){
            $this->msgbox->add('城市不正确', 201);
        }
        ...
        else if($session->get('code_' . $mobile) != $params['sms_code'] &&  ($this->cache->get('sms_code_'.$mobile) != $params['sms_code'] )  ){
            $this->msgbox->add('短信验证码有误', 206); //1111---'.$params['sms_code'].'---'.$mobile
        }
        else if(!$passwd = $params['passwd']){
...
//                $filter['lng'] = $squares['left-bottom']['lng'] . '~' . $squares['right-top']['lng'];
            ...
            $filter[':SQL'] = "((`from`='waimai' AND ((`pei_type`=1 AND `order_status` IN(1,2)) OR (`order_status`=0 AND `pei_type`=2))) OR (`from`='paotui' && `order_status` in (0,1,2)  ))";
            //AND `order_status`=0
            if($new_order_count = (int) K::M('order/order')->count($filter)){
                K::M('staff/staff')->send($this->staff_id, '您有新的订单', '您有新的配送订单', 'order');
                $sql = $this->system->db->SQLLOG();
                K::M('system/logs')->log('__sql_staff_222', array($sql));
            }
