...
class Ctl_Biz_Account extends Ctl
{
    ...
    public function login($params)
    {
        if(!$params['mobile']){
            ...
            if($a = K::M('rongcloud/rongcloud')->init_token('B'.$shop['shop_id'], $shop['title'], $shop['logo'])){
                ...
                $shop['rongcloud'] = array('B'.$shop['shop_id'], 'token'=>'');
            }
    ...
    public function signup($params)
    {
        ...
        if(!$params['mobile']){
            ...
            $this->msgbox->add(L('店铺名称不能为空'),216);
        }else if(!$phone = $params['phone']){
            $this->msgbox->add(L('客服电话不能为空'),217);
        }else if(!$addr = $params['addr']){
            $this->msgbox->add(L('店铺地址不能为空'),218);
        }else if(!$lng = $params['lng']){
            $this->msgbox->add(L('未定位坐标'),219);
        }else if(!$lat = $params['lat']){
            $this->msgbox->add(L('未定位坐标'),220);
        }else if(!$cate_id = (int)$params['cate_id']){
            $this->msgbox->add(L('店铺分类不存在'),221);
        }else if(!$cate = K::M('shop/cate')->detail($cate_id)){
            $this->msgbox->add(L('店铺分类不存在'),222);
        }else if($shop = K::M('shop/shop')->shop($mobile, 'mobile')){
            $this->msgbox->add(L('该手机号已经注册过'),223);
        }else if(!$info = $params['info']){
            $this->msgbox->add('商家公告不能为空',224);
        }else{
            $data = array('cate_id'=>$cate_id, 'contact'=>$contact, 'mobile'=>$mobile, 'title'=>$title, 'phone'=>$phone, 'addr'=>$addr, 'lng'=>$lng, 'lat'=>$lat,'info'=>$info);
            $data['passwd'] = $passwd;
