...
class Ctl_Biz_Comment extends Ctl_Biz
{
    ...
    public function items($params)
    {
        ...
        if(($page<=100) && $items = K::M('shop/comment')->items($filter, array('comment_id'=>'desc'), 1, $limit, $counts)){
        ...
        $this->msgbox->set_data('data', array('items'=>array_values($items), 'total_count'=>$counts,'praise_num'=>$shop['praise_num'],'score_fuwu'=>$shop['score_fuwu'],'score_kouwei'=>$shop['score_kouwei'],'comments'=>$shop['comments']));
    }
    ...
    public function reply($params)
    {
        if(!$comment_id = (int)$params['comment_id']){
            ...
            $shop = K::M('shop/shop')->detail($comment['shop_id']);
            $order = K::M('order/order')->detail($comment['order_id']);
            K::M('order/order')->send_member('商家(' .$shop['title']. ')已回复', '订单(' . $order['order_id'] . ')商家回复内容:' . $reply , $order, null);
            $this->msgbox->add('success');
        }
    }
}