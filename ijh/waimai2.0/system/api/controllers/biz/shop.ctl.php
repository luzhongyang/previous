...
class Ctl_Biz_Shop extends Ctl_Biz
{
    ...
    public function opend()
    {
        if(!$detail = K::M('shop/shop')->detail($this->shop_id)){
            $this->msgbox->add(L('商户不存在或已被删除',211));
        }
        else{
            $data = array(
                'weidian' => $detail['weidian'],
                'pintuan' => $detail['pintuan'],
            );
            $this->msgbox->add('success');
            $this->msgbox->set_data('data', array('detail'=>$data));
        }
    }

    public function opend_set($params)
    {
        if(!$detail = K::M('shop/shop')->detail($this->shop_id)){
            $this->msgbox->add(L('商户不存在或已被删除',211));
        }
        else if(!isset($params['weidian']) && !isset($params['pintuan'])){
            $this->msgbox->add(L('非法的数据提交',212));
        }
        else{
            $data = array();
            if(isset($params['weidian'])){
                $data['weidian'] =  1 == $params['weidian'] ? 1 : 0;
            }
            if(isset($params['pintuan'])){
                $data['pintuan'] =  1 == $params['pintuan'] ? 1 : 0;
            }
    ...
    public function yingye($params)
    {
        ...
        if(!isset($params['yy_status']) || !in_array($params['yy_status'],array(0,1,2))){
    ...
    public function pei($params)
    {
        ...
        if(isset($params['pei_distance'])){ //默认配送3公里
            $data['pei_distance'] = round($params['pei_distance'])>0?round($params['pei_distance']):3;
        }
