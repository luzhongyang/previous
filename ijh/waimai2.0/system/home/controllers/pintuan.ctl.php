...
class Ctl_Pintuan extends Ctl
{
    ...
    public function ajax_index()
    {
        ...
        foreach($arr_product as $k => $val){
            ...
            $arr_product[$k]['discount'] = number_format(($val['tuan_price'] / $val['price']), 2) * 10;
        }
        ...
        if($count > 0){
            foreach($arr_product as $k => $v){
                ...
                $is_collect = 'ok' == $arr_new_collect[$v['pintuan_product_id']] ? 'guanzhu iconfont icon-xinshi-copy' : 'guanzhu iconfont icon-kongxin';
    ...
    public function ajax_search()
    {
        ...
        if($count > 0){
            foreach($arr_product as $k => $v){
                ...
                $intro = K::M('pintuan/product')->intro_short($v['intro']);
                $v['intro'] = $intro;
                $v['link'] = $this->mklink('pintuan:product', $v['pintuan_product_id']);
...
//            $shop_html = K::M('pintuan/collect')->shop_cache_clear($arr_product['shop_id']);//清理缓存
            ...
            $this->pagedata['product'] = $arr_product;
            $this->pagedata['collect'] = $arr_collect;
            $this->tmpl = 'pintuan/product.html';
        }
    }
...
//            }
                ...
                if(!$shop_id = (int) $params['shop_id']){
                    ...
                    if($order_count > 0 || 'single' == $params['order_type']){
                        ...
                        if((int) $params['pei_type'] == 3){  // 自提
                            ...
                            $product_price = $p['price'] * $nums; //单人团,原始价格
                            $freight = $p['ship_fee'];
    ...
    public function tuan_intro()
    {
        ...
        $this->tmpl = 'pintuan/tuan_intro.html';
    }
...
//            
    ...
    public function tuan_join($group_id)
    {
        ...
        $this->pagedata['tpl_title'] = "{$arr_product['title']},特价拼团,只要{$arr_product['tuan_price']}元!";
    ...
    public function tuan_my()
    {
        ...
        $page = max((int) $this->GP('page'), 1);
        $count = $list = array();
        $count['count_0'] = K::M('pintuan/group')->user_group_count($this->uid, 0);
        $count['count_1'] = K::M('pintuan/group')->user_group_count($this->uid, 1);
        $count['count_2'] = K::M('pintuan/group')->user_group_count($this->uid, 2);

        $list['list_0'] = $this->tuan_my_status($this->uid, $status = 0, $page = 1, $limit = 10); //进行中
        $list['list_1'] = $this->tuan_my_status($this->uid, $status = 1, $page = 1, $limit = 10); //完成
        $list['list_2'] = $this->tuan_my_status($this->uid, $status = 2, $page = 1, $limit = 10); //失败
...
//        $status = 0;
        ...
        $count = K::M('pintuan/group')->user_group_count($this->uid, $status);
        ...
        foreach($list as $k => $v){
            ...
            $return_html .= "<li class='one_item'>
                                <div class='list-block media-list border_b border_t'>
                                    <ul>
                                        <li><a
                                           href='" . $this->mklink('pintuan/tuan_detail', $v['pintuan_group_id']) . "' >
                                            <div class='item-content'>
                                                ...
                                                <div class='item-inner'>
                                                    <div class='item-title-row'>
                                                        <div class='item-title overflow_clear'>{$v['product_name']} (团ID:{$group['pintuan_group_id']})
                                                        </div>
                                                    </div>
                                                    <div class='item-subtitle'>
                                                        <div class='pad_t10 pad_b10 pad_r10 pintuan-box'>
                                                            <i class='pintuan'></i>
                                                            <span class='tuan-price'>成团价：￥<big>{$v['product_price']}</big>/件</span><span
                                                                class='youji-blue'><i
                                ...
                                <div class='tuangou-ing border_b'>
                                    <div class='zuo-box fl border_r'><span class='maincl'>{$tuan_notice}
                                    </div>
                                    <div class='you-box'>
                                        <a type='button'
                                           href='" . $this->mklink('pintuan/tuan_detail', $v['pintuan_group_id']) . "'
                                        ...
                                        <a type='button'
                                           href='" . $this->mklink('pintuan/tuan_order_detail', $v['order_id']) . "'
                                           class='pub_btn gray-bd' value='查看订单'>查看订单</a>
                                    </div>
                                    <div class='cl'></div>
                                </div>
                            </li>";
    ...
    public function tuan_my_status($uid, $status, $page = 1, $limit = 30)
    {
        ...
        $page = max((int) $this->GP('page'), 1);
//        echo $status;die;
        $list = K::M('pintuan/group')->user_group_list($this->uid, $status, $page, $limit);
        return $list;
    }
...
//        $status = 0;
                        ...
                        <div class='one_item'>
                                ...
                                <div class='list-block media-list'>
                                    <ul>
                                        <li>
                                            <div class='item-content'>
                                                <div class='item-media'><a href='" . $this->mklink('pintuan/tuan_order_detail', $v['order_id']) . "'><img
                                                        src='/attachs/{$v['photo']}'
                                                        style='width: 4.2rem;'></a></div>
                                                <div class='item-inner'>
                                                    <div class='item-title-row'>
                                                        <div class='item-title overflow_clear'><a href='" . $this->mklink('pintuan/tuan_order_detail', $v['order_id']) . "'>{$v['product_name']}</a></div>
                                                    </div>
    ...
    public function tuan_detail($group_id)
    {
        ...
        if(!in_array($this->uid, $arr_allow_user)){
            $link = $this->mklink('pintuan/tuan_join', $arr_group['pintuan_group_id']);
        ...
        foreach($arr_order as $k => $v){
            ...
            $tuanzhang = 0 == $i ? "<img src='/themes/default/static/images/tuanzhang.png' class='img2'>" : '';
            $shafa = 1 == $i ? "<img src='/themes/default/static/images/shafa.png' class='img2'>" : '';
            echo $master_id . '-' . $this->uid . '<br/>';
            $tichu = 0 == $v['pay_status'] && 0 != $i && $master_id == $this->uid ? "<a href='#' order_id='{$v['order_id']}' class='tichu open-slider-modal'>踢出</a>" : ''; //团长不能踢自己

            $tuan_person_icon .= "
                    <span> 
                    	<img src='/attachs/{$v['face']}' class='img1'> 
                        {$tuanzhang}{$shafa}{$tichu}
                    </span>
                    ";
            ...
            if(0 == $i){
                $tuan_person_html .= "
                        <div class='list1' style=' margin-bottom:0.35rem;'>
                            <img src='/themes/default/static/images/sanjiao.png' width='100%'>
                            <div style=' background:url(/themes/default/static/images/hengtiao.png) repeat-y; background-size:100% auto; padding:0.4rem 0;'>
                                <div class='fl left'>
                                    <div class='fl touxiang'><img src='/attachs/{$arr_user[$v['uid']]['face']}'></div>
                                    <p class='fl'>团长{$arr_user[$v['uid']]['nickname']}</p>
                                    <div class='cl'></div>
                                </div>
                                <div class='fr right mr10'><em class='mr10'>{$dateline_1}</em><span>{$dateline_2}</span>开团</div>
                                <div class='cl'></div>
                            </div>
                        </div>";
            }
                ...
                $tuan_person_html .= "
                        <div style=' border:1px #34ecf8 solid; background:#d6fcfe; position:relative; margin-bottom:0.35rem; padding:0.4rem 0;'>
                            {$shafa_html}
                            <div class='fl left'> 
                                <div class='fl touxiang'><img src='/attachs/{$arr_user[$v['uid']]['face']}'></div>
                                <p class='fl'>{$arr_user[$v['uid']]['nickname']}</p>
                            </div>
                            <div class='fr right mr10'><em class='mr10'>{$dateline_1}</em><span>{$dateline_2}</span>加入</div>
                            <div class='cl'></div>
                        </div>";
            }
            $i++;
