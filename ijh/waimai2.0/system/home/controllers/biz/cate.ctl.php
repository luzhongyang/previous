...
class Ctl_Biz_Cate extends Ctl_Biz
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('waimai/productcate')->items($filter, array('parent_id'=>'ASC','orderby'=>'ASC'), $page, $limit, $count)){
        	$pager['count'] = $count;
        	$pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
        }
        */
        $tree = K::M('waimai/productcate')->tree($this->shop_id);
    ...
    public function create()
    {
        if($data = $this->checksubmit('data')){
            ...
            $this->pagedata['cate_tree'] = K::M('waimai/productcate')->tree($this->shop_id);
    ...
    public function edit($cate_id=null)
    {
        if(!($cate_id = (int)$cate_id) && !($cate_id = $this->GP('cate_id'))){
            ...
            $cate = K::M('waimai/productcate')->detail($cate_id);
            $cate_list = K::M('waimai/productcate')->items(array('parent_id'=>0,'shop_id'=>$cate['shop_id']));
            
            //查找是否有子分类
            $is_have_sub = K::M('waimai/productcate')->find(array('parent_id' => $detail['cate_id']));
            //是否允许编辑上级分类
            $is_edit_parent = 0;
