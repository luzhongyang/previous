...
class Ctl_Biz_index extends Ctl_Biz
{
    ...
    public function index()
    {
        ...
        // 待配送外卖订单数
    	if($wait_peisong = K::M('order/order')->count(array('shop_id'=>$this->shop_id,'order_status'=>1,'pei_type'=>array(0,1,2)))) {
    		$this->pagedata['wait_peisong'] = $wait_peisong;
    	}
        // 今日已完成外卖订单数
        $day = date('Ymd', __TIME);
    	if($today_order = K::M('order/order')->count(array('shop_id'=>$this->shop_id,'order_status'=>array(4,8),'day'=>$day))) {
    		$this->pagedata['today_order'] = $today_order;
    	}
        //总完成外卖订单数
    	if($all_order = K::M('order/order')->count(array('shop_id'=>$this->shop_id,'from'=>'waimai','order_status'=>array(4,8)))) {
