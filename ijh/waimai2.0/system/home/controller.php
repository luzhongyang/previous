...
class Ctl extends Factory
{
    ...
    public function __construct(&$system)
    {
        ...
        if(strpos($_SERVER['HTTP_USER_AGENT'],"WaiMai2.0")){  //判断是否是APP调用WEBVIEW
            $this->pagedata['app_webview'] = 1;
        }
        if(strpos($_SERVER['HTTP_USER_AGENT'],"WaiMai2.0android")){  //判断APP调用是否是安卓
            $this->pagedata['android'] = 1;
        }

        $this->msgbox->template("page/notice.html");
        $this->msgbox->tmpl_miniframe = 'page/miniframe.html';
        $this->msgbox->tmpl_miniload = 'page/miniload.html';
    ...
    {        
        $shop_id = (int) $shop_id;
        $cart_list = (array) json_decode(str_replace("\\\"", "\"", $this->system->cookie->get('ECart')), true);
    ...
    public function check_login()
    {
        if(!$this->uid){
            if($this->request['XREQ'] || $this->request['MINI']){
                $this->msgbox->add('很抱歉，您还没有登录', 101);
            }else{
    ...
    // 微店选择模板
    protected function default_weidian_theme()
    {
        static $theme = null;
        if($theme === null){
            if(empty($theme)){
        ...
        if ($wx_openid === null) {
            if ($code = $this->GP('WXCODE')) {
                ...
                if($wx_unionid = $ret['unionid']){
                    ...
                    $this->err->set_data('forward', $this->mklink('index'));
                }
