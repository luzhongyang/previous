...
class Ctl_Order_Order extends Ctl
{
    ...
    public function detail($order_id = null, $is_delete = 0)
    {
        if (1 == $is_delete) {
            if (!$order_id = (int)$order_id) {
                ...
                if (K::M('order/order')->cancel($order_id, $detail, 'admin')) {
            ...
            if (!$order_id = (int)$order_id) {
                ...
                $waimai_detail = K::M('waimai/order')->detail($order_id);
                $detail = $detail + $waimai_detail;
        ...
        if(!($order_id=(int)$order_id) && !($order_id = (int)$this->GP('order_id'))){
            ...
            if(!$staff = K::M('staff/staff')->detail((int)$data['staff_id'])){
                ...
                K::M('staff/staff')->update_count($staff['staff_id'], 'orders', 1);
                 ...
                 K::M('staff/staff')->send($staff['staff_id'], '您有新的订单', '您有新的配送订单', 'order');
    ...
    public function newcountorder()
    {
        $biz_order_new_count = $staff_order_new_count = 0;
        $filter1 = array('order_status'=>0, 'closed'=>0, 'pay_status'=>1,  'dateline'=> '>:'.strtotime(date('Y-m-d')));
        //$filter1[':SQL'] = 'dateline >'.strtotime(date('Y-m-d'));
        $biz_order_new_count = K::M('order/order')->count($filter1);
		$filter2 = array('staff_id' => 0, 'closed' => 0, 'pay_status' => 1, 'dateline'=> '>:'.strtotime(date('Y-m-d')));
		$filter2[':SQL'] = "((`from`='waimai' AND ((`pei_type`=1 AND `order_status` IN(1,2)) OR (`order_status`=0 AND `pei_type`=2))) OR (`from`='paotui' && `order_status`=0 ))";
//        $filter2 = array('staff_id'=>0, 'pei_type'=>array(1,2), 'order_status'=>array(1,2,3,4,5));
//        $filter2[':SQL'] = 'dateline >'.strtotime(date('Y-m-d'));
        $staff_order_new_count = K::M('order/order')->count($filter2);
        $this->msgbox->set_data('data', array('biz_order_new_count'=>$biz_order_new_count, 'staff_order_new_count'=>$staff_order_new_count));
        $this->msgbox->json();
    }
}