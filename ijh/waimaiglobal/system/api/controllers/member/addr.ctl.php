...
class Ctl_Member_Addr extends Ctl
{
    ...
    public function index($params)
    {
        ...
        if($items = K::M('member/addr')->items($filter, $orderby, $page, 10, $count)){
            foreach($items as $k=>$val){
                $items[$k] = $this->filter_fields('addr_id,uid,contact,mobile,addr,house,is_default,lat,lng', $val);
            }
        }else {
            $items = array();
        }
    ...
    public function update($params)
    {
        ...
        if(!$addr_id = (int)$params['addr_id']){
        ...
        }else if(!K::M('verify/check')->len(strlen($params['contact']), 6, 16)) {
            ...
            if($data = $this->check_fields($params, 'contact,mobile,addr,house,is_default,lat,lng')){
