...
class PaymentModel extends CommonModel {
    ...
    public function respond($code) {
        ...
        if(defined('IN_APP') && IN_APP){
            require_cache(APP_PATH . 'Lib/Payment/' . $code . '.app.class.php');
        ...
        if (!empty($logs) && !$logs['is_paid']) {
            ...
            if (D('Paymentlogs')->save($data)) { //总之 先更新 然后再处理逻辑  这里保障并发是安全的
                ...
                if ($logs['type'] == 'gold') {
                    ...
                    if ($goodslist = D('Goodslist')->where(array('list_id' => $logs['order_id']))->find()) {
                        //更新已众筹数
                        D('Goodstype')->updateCount($goodslist['type_id'],'have_num',1);
                        D('Goodscrowd')->updateCount($goodslist['goods_id'],'have_num',1);

                        D('Goodslist')->where(array('list_id' => $logs['order_id']))->save(array('pay_status'=>1));
                        D('Goodscrowd')->updateCount($goodslist['goods_id'],'have_price',$goodslist['price']);
                    }
                } else { // 商城购物
