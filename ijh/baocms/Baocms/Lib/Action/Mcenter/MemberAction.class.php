...
class MemberAction extends CommonAction {
    ...
    public function pay() {
      ...
      //  D('Lock')->unlock();
        if ($detail['type'] == 'ele') {
            ...
            if (!empty($detail['order_id'])) {
                D('Paymentlogs')->sendWeixinTempMsgByOrderId($detail['order_id'], $this->uid);// 发送微信模板消息
            }elseif (!empty($detail['order_ids'])) {
                $order_id = explode(',', $detail['order_ids']);
                D('Paymentlogs')->sendWeixinTempMsgByOrderId($order_id, $this->uid);// 发送微信模板消息
            }
    ...
    public function index() {
        //处理消息 xuyq
        $this->assign('new_xiaoxi',D('Usermessage')->where(array('user_id'=>$this->uid,'is_read'=>0))->count());
        $this->assign('order', D('Tuanorder')->where(array('user_id' => $this->uid))->count());
        $this->assign('code', D('Tuancode')->where(array('user_id' => $this->uid, 'is_used' => 0))->count());
        ...
        $this->assign('breaks',D('Breaksorder')->where(array('user_id'=>$this->uid))->count());
        $lists = D('Shopyuyue')->where(array('user_id'=>$this->uid,'used'=>0))->select();
        foreach ($lists as $k=>$val){
            if(strtotime($val['yuyue_date'].$val['yuyue_time']) < NOW_TIME){
                unset($lists[$k]);
            }
        }
        $this->assign('yuyue',count($lists));
