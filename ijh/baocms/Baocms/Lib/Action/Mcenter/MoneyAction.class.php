...
class MoneyAction extends CommonAction {
    ...
    public function fzmoneypay() { //后期优化
        ...
        header("Location:" . U('mobile/payment/payment', array('log_id' => $logs['log_id'])));
        die;
    }
    ...
    public function moneypay() { //后期优化
        ...
        $back_url = $this->_post('back_url','htmlspecialchars');
        $logs = array(
            'user_id' => $this->uid,
            'type' => 'money',
            'code' => $code,
        ...
        header("Location:" . U('mobile/payment/payment', array('log_id' => $logs['log_id'],'back_url'=>$back_url)));
        die;
    }
    
    
    public function app_moneypay() {
        $money = (int) ($this->_post('money') * 100);
        if ($money <= 0) {
             $this->ajaxReturn(array('status'=>'error','message'=>'请填写正确的充值金额!'));
        }
        $logs = array(
            'user_id' => $this->uid,
            'type' => 'money',
        ...
        $this->ajaxReturn(array('status'=>'success','message'=>'下单成功!','order_id'=>$logs['log_id'],'amount'=>round($money/100,2),'url'=>$url));
    ...
    public function app_goldpay() { //后期优化
        $gold = (int) $this->_post('gold');
        if ($gold <= 0) {
            $this->ajaxReturn(array('status'=>'error','message'=>'请填写正确的金块数!'));
        }
        ...
        $logs['log_id'] = D('Paymentlogs')->add($logs);
        $url = "http://".$_SERVER['HTTP_HOST']."/mcenter/";
        $this->ajaxReturn(array('status'=>'success','message'=>'下单成功!','order_id'=>$logs['log_id'],'amount'=>$gold,'url'=>$url));
