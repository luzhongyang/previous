...
class CrowdAction extends CommonAction
{
    ...
    {
        $this->assign('nextpage', LinkTo('crowd/loaddata', array('goods_id' => $goods_id, 't' => NOW_TIME, 'p' => '0000')));
    ...
    public function confirm_goods($list_id=0){
        if (!$list_id = (int) $list_id) {
            $this->error('未选择要操作的内容ID');
        }elseif (!$detail = D('Goodslist')->find($list_id)) {
            $this->error('您要操作的内容不存在或已被删除');
            ...
            $this->success('确认收货成功！',U('crowd/detail', array('goods_id' => $detail['goods_id'])));
        }
    }
