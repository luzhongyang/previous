...
class PaymentAction extends CommonAction {
    ...
    public function yes($log_id){
        ...
        switch ($logs['type']){
            ...
            case 'ding':
                ...
                break;
            case 'crowd':
                $this->crowd_success('恭喜您支付成功啦！', $logs);
                break;
            case 'hotel':
                $this->hotel_success('恭喜您支付成功啦！', $logs);
                break;
            case 'tuan':
    ...
    public function payment($log_id,$back_url=null) {
        if (empty($this->uid)) {
            AppJump();
        }
        $log_id = (int) $log_id;
