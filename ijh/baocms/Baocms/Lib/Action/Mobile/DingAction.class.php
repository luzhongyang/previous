   ...
   public function index() {
	...
	public function detail($shop_id)
	{
        ...
        if(!$shop_id = (int)$shop_id){
            ...
            //特色菜
            $menus = D('Shopdingmenu')->where(array('shop_id'=>$shop_id,'is_tuijian'=>1))->limit(8)->select();
            ...
            $more_list = $shopding->where($filter)->limit(2)->select();
    ...
    public function orderCreate($shop_id){
        ...
        if (empty($ding_time)) { $ding_time = $ding_date;}
		if (empty($ding_time)) { 
        ...
        if($order_id = D('Shopdingorder')->add($data)){
            ...
            cookie('mobile',null);
            cookie('name',null);
            $url = U('/mcenter/ding/detail', array('order_id' => $order_id));
            if(!$amount){
                $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功！','order_id'=>$order_id,'to_detail'=>1));
            }else{
                $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'amount'=>$amount/100,'url'=>$url));
            }
        }else{
