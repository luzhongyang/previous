...
class MartAction extends CommonAction {
    ...
    public function app_pay() {
        ...
        if (empty($addr_id)) {
            $this->ajaxReturn(array('status'=>'error','message'=>'请选择一个要配送的地址'));
        }
        ...
        if ($code == 'wait') { //如果是货到付款
            $this->ajaxReturn(array('status'=>'error','message'=>'1111111111'));
            D('Order')->save(array(
                'order_id' => $order_id,
                'is_daofu' => 1,
            ));
            ...
            /*微信订单通知消息-结束*/
                ...
                $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'amount'=>round($order['total_price']/100,2),'url'=>$notice_data['url']));
        }
    }
