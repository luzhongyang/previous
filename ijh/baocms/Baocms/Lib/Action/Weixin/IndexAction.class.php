...
class IndexAction extends CommonAction {
    ...
    public function index() {
        ...
        //file_put_contents('/www/web/b_baocms_cn/public_html/Baocms/Lib/Action/Weixin/aaa.txt', var_export($data, true));
        baolog('weixin.data', array($data));
        switch ($data['MsgType']) {//
            case 'event':
    ...
    private function event() {
        if ($this->shop_id == 0) {
            ...
            baolog('weixin.event.gz', array(__FILE__.__LINE__, $data, $weixin_msg));
            if ($weixin_msg['type'] == 1) {
                ...
                baolog('weixin.event.gz', array(__FILE__.__LINE__, $data, $content));
                $this->weixin->response($content, 'news');
            }
        }
    }
