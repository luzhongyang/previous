...
class RelayAction extends CommonAction {
	...
	//$openid = 111111;
			...
			$weixin =D('User_weixin')->where(array('openid'=>$openid))->find();
			if($weixin['user_id']){
				$member = D('User')->find($weixin['user_id']);
			}else{
				$this->redirect("Relay/preview",array('relay_id'=>$relay_id));
			}
	...
	public function sign($relay_id,$qian,$sn_id=null)
	{
		...
		//$openid = '111111';
				...
				$member = D('Users')->find($weixin['user_id']);
				//$member =  K::M('member/weixin')->detail_by_openid($openid);
				$stime = strtotime($detail ['stime']);
				$ltime = strtotime($detail ['ltime']);
				if (! empty ( $ltime ) && $ltime <= time ()) {
					$this->baoreturn('1','您来晚啦');
					//$this->err->add('您来晚啦', 215);
				}else if ($detail ['follower_condtion'] && $wx_info['subscribe'] == 0) {

					switch ($detail ['follower_condtion']) {
						case 1 :
							$this->baoreturn('1','关注后才能领取');
							//$this->err->add('关注后才能领取', 214);
							break;
					}
				}else if ($detail ['member_condtion'] == 1 && !$member['uname']) {
					$this->baoreturn('1','用户注册后才能领取');
					//$this->err->add('用户注册后才能领取', 213);
				}else {
					if($qian >= $detail['owner_maxgold']){
						$qian = $detail['owner_maxgold'];
					}
					$data = array();
					$data['cishu'] = 1;
					...
					if($sn = $objsn->add($data)){
						...
						$this->baoreturn('1','您已参与该商家的另一接力活动！');
					}
				}
			}else{
				if(empty($openid)){
	...
	//$openid = '22222222222';
				...
				foreach($list as $k => $v){
					if($v['cishu']>= $detail['max_num']){
					...
					}else if($v['openid'] != $openid){$this->baoreturn('0',1111);
						$this->baoreturn('0','用户错误');
						//$this->Error('用户错误');
						//$this->err->add('用户错误', 214);
					}else if($v['gold'] >= $detail['owner_maxgold']){
						//$this->Error('您已经到金币上限了');
						$this->baoreturn('1','已经到金币上限了');
						...
						$a = $objsn->find($sn_id);
						$b['gold'] = $a['gold'] + $qian;
						if($b['gold'] >= $detail['owner_maxgold']){
							$qian = $detail['owner_maxgold'] - $b['gold'];
							$b['gold'] = $detail['owner_maxgold'];
						}
	...
	{ 
		...
		$objsn = D('Weixin_relaysn');
		if(!($relay_id = (int)$relay_id) && !($relay_id = $this->GP('relay_id'))){
			$this->baoreturn('1','未指定该接力ID');
        }else if(!$detail = $obj->find($relay_id)){
			$this->baoreturn('1','该接力不存在或已经删除');
			//$this->err->add('该接力不存在或已经删除', 216);
        }else if(!$relaysn = $objsn->find($sn_id)){
			$this->baoreturn('1','该接力玩家不存在');
	...
	//$openid = '99999999999999';
			...
			if($my_count >= $detail['relay_num']){
				$this->baoreturn('1','您已经没有接力次数了');
	
				//$this->err->add('您已经没有接力次数了', 213);
			}else if($aa['gold'] >= $detail['owner_maxgold']){
				$this->baoreturn('1','金币已达上限了！');
			}else{
				$bb['gold'] = $aa['gold'] + $qian;
				if($bb['gold'] >= $detail['owner_maxgold']){
					$qian = $detail['owner_maxgold'] - $bb['gold'];
					$bb['gold'] = $detail['owner_maxgold'];
				}
				$bb['sn_id'] = $aa['sn_id'];
				$objsn->save($bb);
				//K::M('weixin/relaysn')->update_count($sn_id, 'gold', $qian);
