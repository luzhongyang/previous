...
class OrderAction extends CommonAction {
    ...
    public function pick() {
        ...
        foreach ($data as $k=>$v) {
            $v['order_id'] = $k;
            if($v['order_id'] && $v['deliver'] && $v['num']){
                $v['datetime'] = $time;
                if(D('Order_delivery')->add($v)){
                    $order = D('Order')->find($k);
                    D('Paymentlogs')->sendWeixinTempMsgByOrderId($order['order_id'], $order['user_id'], '亲，您的订单已发货，我们会第一时间内为您安排专人免费送货到家！订单详情如下：');// 发送微信模板消息
                    D('Order')->save(array('status' => 2,'update_time'=>$time), array("where" => array('order_id' =>$v['order_id'])));
                    D('Ordergoods')->save(array('status' => 1), array("where" => array('order_id' => $v['order_id'])));
                }else{
                    $error = true;
                    break;
                }
    ...
    */
    ...
    public function delivery() {
        ...
        $show = $Page->show(); // 分页显示输出
        $list = $Order->where($map)->order(array('order_id' => 'asc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        // print_r($count);print_r($list);die;
        ...
        if (!empty($order_ids)) {
            $goods = D('Ordergoods')->where(array('order_id' => array('IN', $order_ids)))->select();
            $delivery = D('Order_delivery')->where(array('order_id' => array('IN', $order_ids)))->select();
            $goods_ids = array();
            foreach ($goods as $val) {
                $goods_ids[$val['goods_id']] = $val['goods_id'];
            }
            foreach ($delivery as $k => $v) {
                $deliverys[$v['order_id']] = $v;
            }
