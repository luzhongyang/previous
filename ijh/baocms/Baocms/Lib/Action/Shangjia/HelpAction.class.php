...
class HelpAction extends CommonAction {
	...
	private $create_fields = array('help_id','shop_id','keyword_id','keyword','title','intro','photo','stime','ltime','use_tips','end_tips','predict_num','max_num','follower_condtion','member_condtion','collect_count','views','end_photo','clientip','dateline');
	private $edit_fields = array('help_id','shop_id','keyword_id','keyword','title','intro','photo','stime','ltime','use_tips','end_tips','predict_num','max_num','follower_condtion','member_condtion','collect_count','views','end_photo','clientip','dateline');
	...
	public function create()
	{
		...
		if ($this->isPost()) {
			...
			if ($id = $obj->add($data)) {
                            $url = U('weixin/help/preview', array('help_id' => $id));
                            $url = __HOST__ . $url;
	...
	public function edit($help_id = null) {
        if ($help_id = (int) $help_id) {
            ...
            if ($this->isPost()) {
                ...
                $map['keyword'] = $data['keyword'];
                if($k = $objk->where($map)->select()&&$k['help_id'] == $help_id){
                    $this->baoError('该关键字已经被使用，请修改关键字');
                }else{
                    if($detail['keyword_id']){
                        ...
                        $url = U('weixin/help/preview', array('help_id' => $help_id));
                        $url = __HOST__ . $url;
                        $data1['url'] = $url;
                        $objk->save($data1);
                    }else{
