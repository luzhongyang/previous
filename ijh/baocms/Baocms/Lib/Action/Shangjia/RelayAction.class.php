...
class RelayAction extends CommonAction {
	private $create_fields = array('help_id','owner_maxgold','shop_id','keyword_id','keyword','title','intro','photo','relay_num','stime','ltime','time','use_tips','end_tips','predict_num','max_gold','min_gold','max_num','follower_condtion','member_condtion','collect_count','views','end_photo','clientip','dateline');
	private $edit_fields = array('help_id','owner_maxgold','shop_id','keyword_id','keyword','title','intro','photo','relay_num','stime','ltime','time','use_tips','end_tips','predict_num','max_gold','min_gold','max_num','follower_condtion','member_condtion','collect_count','views','end_photo','clientip','dateline');
	...
	public function create()
	{
		...
		if ($this->isPost()) {
			...
			if ($id = $obj->add($data)) {
                            $url = U('weixin/relay/preview', array('relay_id' => $id));
                            $url = __HOST__ . $url;
	...
	private function createCheck() {
		...
		if (empty($data['owner_maxgold'])) {
            $this->baoError('玩家金币上限不能为空！');
        }
	...
	public function edit($relay_id = null) {
        if ($relay_id = (int) $relay_id) {
            ...
            if ($this->isPost()) {
                ...
                $map['keyword'] = $data['keyword'];
                if($k = $objk->where($map)->select()&&$k['relay_id'] == $relay_id){
                    $this->baoError('该关键字已经被使用，请修改关键字');
                }else{
                    if($detail['keyword_id']){
                        ...
                        $url = U('weixin/relay/preview', array('relay_id' => $relay_id));
                        $url = __HOST__ . $url;
                        $data1['url'] = $url;
                        $objk->save($data1);
                    }else{
	...
	public function sndelete($sn_id=null)
    {
        ...
        if($sn_id = (int)$sn_id){
            if(!$detail = $obj->find($sn_id)){
                ...
                if($obj->delete($sn_id)){
                    $this->baoSuccess('删除内容成功!',U('relay/sn',array('relay_id'=>$detail['relay_id'])));
                    //$this->err->add('删除内容成功', 211);
                }
            }
