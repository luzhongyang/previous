...
class PacketAction extends CommonAction
{
    ...
    private $create_fields = array('id', 'shop_id','keyword_id', 'title', 'item_min', 'title', 'keyword', 'msg_pic', 'desc', 'info', 'start_time', 'end_time', 'ext_total', 'get_number', 'value_count', 'is_open', 'item_num', 'item_sum', 'item_max', 'item_unit', 'packet_type', 'deci', 'people', 'password');

    private $edit_fields = array('id', 'shop_id','keyword_id', 'title', 'item_min', 'title', 'keyword', 'msg_pic', 'desc', 'info', 'start_time', 'end_time', 'ext_total', 'get_number', 'value_count', 'is_open', 'item_num', 'item_sum', 'item_max', 'item_unit', 'packet_type', 'deci', 'people', 'password');
    ...
    public function create()
    {
        ...
        if($this->isPost()){
            ...
            if($id = $obj->add($data)){
                $url = U('weixin/packet/index', array('packet_id' => $id));
                $url = __HOST__ . $url;
            ...
            if($this->isPost()){
                ...
                if($k = $objk->where($map)->select()&&$k['id'] == $id){
                    ...
                    if($detail['keyword_id']){
                        $data1 = array('keyword_id'=>$detail['keyword_id'],'keyword'=>$data['keyword'],'title'=>$data['title'],'contents'=>$data['info'],'photo'=>$data['msg_pic']);
                        $url = U('weixin/packet/index', array('packet_id' => $id));
                        $url = __HOST__ . $url;
                        $data1['url'] = $url;
                        $objk->save($data1);
                    }else{
                        $data2 = array('type'=>'addon','keyword'=>$data['keyword'],'title'=>$data['title'],'contents'=>$data['info'],'photo'=>$data['msg_pic'],'shop_id'=>$this->shop_id);
                        $url = U('weixin/packet/index', array('packet_id' => $id));
                        $url = __HOST__ . $url;
                        $data2['url'] = $url;
