...
class MallAction extends CommonAction
{
    ...
    private function stop_crowd(){
        //$this->display('stop');die;
    }
    ...
    public function crowdDetail($goods_id)
    {
        ...
        foreach ($goodstype as $k => $v) {
            if (!empty($v['max_num']) && $v['have_num'] > $v['max_num']) {
                $goodstype[$k]['have_num'] = $v['max_num'];
            }
        }
        $this->assign('goodstype', $goodstype);

        //获取购买记录
        //$listcount = $Goodslist->where(array('goods_id' => $goods_id, 'pay_status'=>1))->count();
        $goods_list = $Goodslist->where(array('goods_id' => $goods_id, 'pay_status'=>1))->order(array('dateline' => 'desc'))->select();
    ...
    public function buy($goods_id=0)
    {
        ...
        if (!$goods_id = (int) $goods_id) {
            $this->baoError('未选择要购买的众筹');
        }elseif (!$detail = D('Goods')->find($goods_id)) {
            $this->baoError('该商品不存在');
        }elseif (!$data = $this->check_fields($this->_post('data'), 'type_id,name,mobile,addr')) {
            $this->baoError('非法的参数提交');
        }elseif (!$type = D('Goodstype')->where(array('type_id'=>(int)$data['type_id'], 'goods_id'=>$goods_id))->find()) {
            $this->baoError('该商品不存在');
        }elseif (!empty($type['max_num']) && $type['have_num'] >= $type['max_num']) {
            $this->baoError('该众筹限额已满，无法购买');
        }elseif (!$data['name'] = trim(htmlspecialchars($data['name']))) {
            $this->baoError('姓名不能为空');
        }elseif (!$data['mobile'] = $data['mobile']) {
            $this->baoError('手机号码不能为空');
        }elseif (!$data['addr'] = trim(htmlspecialchars($data['addr']))) {
            $this->baoError('地址不能为空');
        }else{
            $data['uid'] = $this->uid;
            $data['price'] = $type['price'];
            $data['total_price'] = $type['price'];
            $data['goods_id'] = $goods_id;
            $data['dateline'] = NOW_TIME;
            if (!empty($type['yunfei'])) {
                $data['total_price'] += $type['yunfei'];
            }
            if($list_id = D('Goodslist')->add($data)){
                $this->baoSuccess('众筹订单创建成功',U('mall/crowdpay', array('list_id' => $list_id)));
            }
        }
    ...
    public function crowdpay($list_id){
        $this->stop_crowd();
        if(!$list_id = (int)$list_id){
            $this->error('众筹订单不存在');
        }elseif(!$list = D('Goodslist')->find($list_id)){
            $this->error('众筹订单不存在');
        }elseif($list['uid'] != $this->uid || $list['order_status'] != 0){
            $this->error('非法操作订单');
        }elseif ($list['pay_status'] == 1) {
            $this->error('该订单已经支付');
        }elseif (!$type = D('Goodstype')->where(array('type_id'=>$list['type_id'], 'goods_id'=>$list['goods_id']))->find()) {
            $this->error('该商品不存在');
        }elseif (!empty($type['max_num']) && $type['have_num'] >= $type['max_num']) {
            $this->error('该众筹限额已满，无法购买');
        }else{
    ...
    public function crowdpay2($list_id){
        $this->stop_crowd();
        if (empty($this->uid)) {
            $this->ajaxLogin();
        }
        if(!$list_id = (int) $list_id){
            $this->baoError('该众筹订单不存在');
        }elseif(!$list = D('Goodslist')->find($list_id)){
            $this->baoError('该众筹订单不存在');
        }elseif($list['uid'] != $this->uid || $list['order_status'] != 0){
            $this->baoError('非法操作订单');
        }elseif ($list['pay_status'] == 1) {
            $this->baoError('该订单已经支付');
        }elseif (!$type = D('Goodstype')->where(array('type_id'=>$list['type_id'], 'goods_id'=>$list['goods_id']))->find()) {
            $this->baoError('该商品不存在');
        }elseif (!empty($type['max_num']) && $type['have_num'] >= $type['max_num']) {
            $this->baoError('该众筹限额已满，无法购买');
        ...
        $logs = D('Paymentlogs')->getLogsByOrderId('crowd', $list_id);
        if (empty($logs)) {
            $logs = array(
                'type' => 'crowd',
                'user_id' => $this->uid,
            ...
            $logs['need_pay'] = $list['total_price'];
            $logs['code'] = $code;
            ...
            /*微信订单通知消息-开始*/
            $notice_data = array(
                'url'       =>  "http://".$_SERVER['HTTP_HOST']."/mcenter/goods/detail/order_id/".$list_id.".html",
                'first'   => '亲,您的订单创建成功!',
                'remark'  => '详情请登录-http://'.$_SERVER['HTTP_HOST'],
                'amount'  => round($list['total_price']/100,2).'元',
                'order'   => $list_id,
                'info'    => $goods['title']
            );
            ...
            foreach($cart_goods as $k => $v){
                ...
                if($vid){
                    ...
                    if($value['mall_price'] > $value['price']){
                        $aa = $value['mall_price'];
                        $value['mall_price'] = $value['price'];
                        $value['price'] = $aa;
                    }
                    $total_price += $value['mall_price'] * (int) $v;
                }
                else{
                    if($row['mall_price'] > $row['price']){
                        $aa = $row['mall_price'];
                        $row['mall_price'] = $row['price'];
                        $row['price'] = $aa;
                    }
    ...
    public function detail($goods_id)
    {
        ...
        if($goods_value_list = D('Goodsformat')->where(array('goods_id' => $goods_id))->select()){
            ...
            if($goods_value['price'] < $goods_value['mall_price']){
                $cc = $goods_value['price'];
                $goods_value['price'] = $goods_value['mall_price'];
                $goods_value['mall_price'] = $cc;
            }
    ...
    public function cartadd()
    {
        ...
        if($num > $detail['kucun']){
            $this->ajaxReturn(array('status' => 'error', 'msg' => '商品库存不足' . $detail['kucun']));
        }
        if($num > 99){
            $num = 99;
        }
    ...
    public function order()
    {
        ...
        foreach($order_goods as $sid => $val){
            ...
            //$a['can_use_integral'] = $val['use_integral'];
        ...
        if(count($order_ids) > 1){//如果大于1 那么形成一个 支付记录 来合并付款！如果其他条件可以直接去付款
            ...
            $logs = array(
                ...
                'user_id'     => $this->uid,
                'order_id'    => 0,
                'order_ids'   => join(',', $order_ids).',',//便于模糊查询使用
                'code'        => '',
