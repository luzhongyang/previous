...
class EleAction extends CommonAction {
    ...
    public function order() {
        ...
        if ($order_id = D('Eleorder')->add($order_data)) {
            ...
            cookie('ele', null);
            $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'order_id'=>$order_id,'amount'=>($val['price'] * $val['buy_num']),'url'=>U('ele/pay', array('order_id' => $order_id))));
        }
    ...
    public function app_pay() {
        ...
        if ($code == 'wait') { //如果是货到付款
            ...
            $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'amount'=>round($order['need_pay']/100,2),'url'=>$notice_data['url']));
        }
    }
