...
class WeidianAction extends CommonAction {
    ...
    public function index() {
        ...
        $this->assign('city_id', $this->_fenzhan['city_id']);
    ...
    public function audit($wd_id = 0) {
        if (is_numeric($wd_id) && ($wd_id = (int) $wd_id)) {
            ...
            if(!$detail = $obj->find($wd_id)){
                $this->baoError('数据不存在或已删除');
            }elseif($detail['city_id'] != $this->_fenzhan['city_id']){
                $this->baoError('不能操作其他城市数据');
            }
    ...
    public function select(){
        ...
        import('ORG.Util.Page'); // 导入分页类
        $map = array('audit'=>1,'city_id'=>$this->_fenzhan['city_id']);
