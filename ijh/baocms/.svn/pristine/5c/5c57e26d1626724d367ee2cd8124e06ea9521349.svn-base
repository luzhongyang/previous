...
class ShopdingAction extends CommonAction {
    ...
    public function index(){
        $this->check_ding();
        //D('Shopdingorder')->plqx($this->shop_id);
    ...
    public function set_ding(){
        ...
        if ($this->isPost()) { 
            ...
            $time = explode('-', $data['business_time']) ;
            $data['stime'] = $time['stime'] ;
            $data['ltime'] = $time['ltime'] ;
            if (empty($shopding)) {
                if($obj->add($data)){
                    foreach($thumb as $k=>$val){
                        D('Shopdingpics')->add(array('shop_id'=>$this->shop_id,'photo'=>$val));
                    }
                    foreach($type as $k=>$val){
                        D('Shopdingattr')->add(array('shop_id'=>$this->shop_id,'type_id'=>$val));
                    }
                ...
                if(false !== $obj->save($data)){
                    D('Shopdingpics')->where(array('shop_id'=>$this->shop_id))->delete();
                    foreach($thumb as $k=>$val){
                        D('Shopdingpics')->add(array('shop_id'=>$this->shop_id,'photo'=>$val));
                    }
                    D('Shopdingattr')->where(array('shop_id'=>$this->shop_id))->delete();
                    foreach($type as $k=>$val){
                        D('Shopdingattr')->add(array('shop_id'=>$this->shop_id,'type_id'=>$val));
                    }
            ...
            $thumb = D('Shopdingpics')->where(array('shop_id'=>$this->shop_id))->select();
            $this->assign('thumb', $thumb);
            $have_type = D('Shopdingattr')->where(array('shop_id'=>$this->shop_id))->select();
            $typess = array();
