...
class EleAction extends CommonAction {
    ...
    public function index() {
        import('ORG.Util.Page'); // 导入分页类
        $ele = D('Ele');
        $map = array('city_id' => $this->city_id,'is_open'=>1);
        $linkArr = array();
    ...
    public function detail() {
        ...
        $tuan = D('Tuan')->where(array('shop_id' => $shop_id, 'audit' => 1, 'closed' => 0, 'end_date' => array('EGT', NOW)))->order(' tuan_id desc ')->limit(0, 5)->select();
