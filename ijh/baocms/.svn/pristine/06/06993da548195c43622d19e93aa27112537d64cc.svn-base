...
class FarmAction extends CommonAction {
   ...
   public function _initialize() {
    ...
    public function index() {
        ...
        $count = $farm->where($map)->count(); // 查询满足要求的总记录数 
    ...
    public function yuyue() {
        ...
        $Page = new Page($count, 25); // 实例化分页类 传入总记录数和每页显示的记录数
        $show = $Page->show(); // 分页显示输出
        $list = $farm->where($map)->order(array('order_id' => 'desc'))->limit($Page->firstRow . ',' . $Page->listRows)->select();
        $farm_ids = $package_ids = array();
        foreach($list as $k => $v){
            $farm_ids[$v['farm_id']] = $v['farm_id'];
            $package_ids[$v['pid']] = $v['pid'];
        }
        $farms = D('Farm')->itemsByIds($farm_ids);
        $packs = D('FarmPackage')->itemsByIds($package_ids);
        foreach($list as $k => $v){
            $list[$k]['farm'] = $farms[$v['farm_id']];
            $list[$k]['farmpackage'] = $packs[$v['pid']];
        }
        $this->assign('list', $list); // 赋值数据集
        $this->assign('page', $show); // 赋值分页输出
        $this->display(); // 输出模板
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->cancel($order_id)){
                    $this->baoSuccess('订单取消成功',U('farm/yuyue'));
                }else{
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->complete($order_id)){
                    $this->baoSuccess('订单操作成功',U('farm/yuyue'));
                }else{
        ...
        if($order_id = (int) $order_id){
            if(!$order = D('FarmOrder')->find($order_id)){
                ...
                if(false !== D('FarmOrder')->save(array('order_id'=>$order_id,'closed'=>1))){
                    $this->baoSuccess('订单删除成功',U('farm/yuyue'));
                }else{
