...
class LifeAction extends CommonAction {
    ...
    private $create_fields = array('title', 'cate_id', 'city_id', 'area_id', 'business_id', 'user_id', 'is_shop', 'text1', 'text2', 'text3', 'num1', 'num2', 'select1', 'select2', 'select3', 'select4', 'select5', 'urgent_date', 'top_date', 'photo', 'contact', 'mobile', 'qq', 'addr', 'views', 'lng', 'lat');
    ...
    private function createCheck() {
        ...
        if($data['city_id'] != $this->_fenzhan['city_id']){
            $this->baoError('城市必须为当前城市');
        }
    ...
    public function delete($life_id = 0) {
        if (is_numeric($life_id) && ($life_id = (int) $life_id)) {
            $obj = D('Life');
            $life = $obj->find($life_id);
            if($life['city_id'] != $this->_fenzhan['city_id']){
                $this->baoError('该信息不属于当前城市');
            }
            $obj->delete($life_id);
