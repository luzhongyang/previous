...
class HdmobileAction extends CommonAction {
    ...
    public function createCheck() {
        ...
        $data['city_id'] = $this->_fenzhan['city_id'];
        if (empty($data['title'])) {
    ...
    public function delete($huodong_id = 0) {
        if (is_numeric($huodong_id) && ($huodong_id = (int) $huodong_id)) {
            $obj = D('Huodong');
            $huodong = $obj->find($huodong_id);
            if($huodong['city_id'] != $this->_fenzhan['city_id']){
    ...
    private function editCheck() {
        $data = $this->checkFields($this->_post('data', false), array('title', 'addr','limit_num','traffic','intro', 'sex', 'photo', 'time', 'cate_id', 'shop_id', 'user_id'));
        $data['user_id'] = (int) $data['user_id'];
        $data['cate_id'] = (int) $data['cate_id'];
        $data['lng'] = htmlspecialchars($data['lng']);
        $data['lat'] = htmlspecialchars($data['lat']);
