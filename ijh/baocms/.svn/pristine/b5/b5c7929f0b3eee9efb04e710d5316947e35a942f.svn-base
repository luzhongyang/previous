...
class  CommonAction extends  Action{
    ...
    {
        return $this->admin_wechat_client();
        if(!$client = D('Weixin')->wechat_client($shop_id)){
            exit('网站公众号设置错误');
        }
        return $client;
    }

    protected  function  admin_wechat_client()
    {
        static $client = null;
        if($client === null){
            if(!$client = D('Weixin')->admin_wechat_client()){
                //var_dump($client);
                exit('网站公众号设置错误');
            }
        }
       ...
       static $openid = null;
        if($force || $openid === null){
            if($code = $_REQUEST['code']){   
                $client = $this->admin_wechat_client();
