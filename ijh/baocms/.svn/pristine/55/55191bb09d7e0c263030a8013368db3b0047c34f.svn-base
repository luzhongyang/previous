...
class ShopmoneyAction extends CommonAction{
    ...
    public function tjmonth(){
       ...
       import('ORG.Util.Page');// 导入分页类
        ...
        $count      = $Shopmoney->tjmonthCount($month,$map['shop_id']);// 查询满足要求的总记录数 
        $Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
        $show       = $Page->show();// 分页显示输出
        $list = $Shopmoney->tjmonth($month,$map['shop_id'],$Page->firstRow,$Page->listRows);
        $shop_ids = array();
        foreach($list as  $val){
            $shop_ids[$val['shop_id']] = $val['shop_id'];
        }
    ...
    public function tjyear(){
       ...
       import('ORG.Util.Page');// 导入分页类
        ...
        $count      = $Shopmoney->tjyearCount($year,$map['shop_id']);// 查询满足要求的总记录数 
        $Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
        $show       = $Page->show();// 分页显示输出
        $list = $Shopmoney->tjyear($year,$map['shop_id'],$Page->firstRow,$Page->listRows);
        $shop_ids = array();
        foreach($list as  $val){
            $shop_ids[$val['shop_id']] = $val['shop_id'];
        }
    ...
    public function tjday(){
       ...
       import('ORG.Util.Page');// 导入分页类
        ...
        $count      = $Shopmoney->tjdayCount($day,$map['shop_id']);// 查询满足要求的总记录数 
        $Page       = new Page($count,15);// 实例化分页类 传入总记录数和每页显示的记录数
        $show       = $Page->show();// 分页显示输出
        $list = $Shopmoney->tjday($day,$map['shop_id'],$Page->firstRow,$Page->listRows);
        $shop_ids = array();
        foreach($list as  $val){
            $shop_ids[$val['shop_id']] = $val['shop_id'];
        }
