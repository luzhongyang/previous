...
class TuanAction extends CommonAction {
    ...
    public function push() {  // 这里的代码在mobile首页被调用。新版6.0重新编写
        ...
        if ($this->uid) {
            ...
            if (count($looks) > 20) { //保险起见
                ...
                $tuans = D('Tuan')->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where(array('shop_id' => array('IN', $shop_ids), 'closed' => 0,'city_id'=>$this->city_id, 'audit' => 1, 'bg_date' => array('ELT', NOW), 'end_date' => array('EGT', NOW)))->limit(0, 10)->select();
            } else {
                $tuans = D('Tuan')->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where(array('closed' => 0,'city_id'=>$this->city_id, 'audit' => 1, 'bg_date' => array('ELT', NOW), 'end_date' => array('EGT', NOW)))->limit(0, 10)->select();
            }
        } else {
            $tuans = D('Tuan')->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where(array('closed' => 0,'city_id'=>$this->city_id, 'audit' => 1, 'bg_date' => array('ELT', NOW), 'end_date' => array('EGT', NOW)))->limit(0, 10)->select();
        }
    ...
    public function detail() {
        ...
        $this->assign('tuans', D('Tuan')->where(array('audit' => 1, 'closed' => 0, 'shop_id' => $shop_id, 'bg_date' => array('ELT', NOW), 'end_date' => array('EGT', NOW), 'tuan_id' => array('NEQ', $tuan_id)))->limit(0, 5)->select());
    ...
    public function order() {
        ...
        if ($order_id = D('Tuanorder')->add($data)) {
            ...
            /*微信订单通知用户消息-结束*/
            //====================微信支付通知==抢购=========================
            header("Location:" . __HOST__.U('tuan/pay', array('order_id' => $order_id)));
            // header("Location:http://www.baocms.cn/mobile" );
            die;
        }
        $this->error('创建订单失败！');
    }
   ...
   //          AppJump();
   //      }
   //      $tuan_id = (int) $this->_post('tuan_id');
   //      if (!$detail = D('Tuan')->find($tuan_id)) {
   ...
   //      if($detail['limit'] > 0){
   //          if($orders >= $detail['limit']){
   //              $this->ajaxReturn(array('status'=>'error','msg'=>'抱歉，该商品每人限购'.$detail['limit'].'件'));
   //              die;
   //          }else if($num > ($detail['limit'] - $orders)){
   //              $this->ajaxReturn(array('status'=>'error','msg'=>'抱歉，该商品每人限购'.$detail['limit'].'件'));
   //              die;
   ...
   //          /*微信订单通知用户消息-结束*/
   //          //====================微信支付通知==抢购=========================
   //          //$url = U('/mcenter/tuan/detail', array('order_id' => $order_id));
   //          $url = __HOST__.U('tuan/pay', array('order_id' => $order_id));

   //          $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'amount'=>(round($data['need_pay']/100,2)),'url'=>$url));
   ...
   //  }
    ...
    public function near() {
        ...
        $tuans = D('Tuan')->order(" (ABS(lng - '{$lng}') +  ABS(lat - '{$lat}') ) asc ")->where(array('closed' => 0, 'audit' => 1, 'bg_date' => array('ELT', NOW), 'end_date' => array('EGT', NOW)))->limit(0, 4)->select();
        foreach ($tuans as $k => $val) {
            $tuans[$k]['d'] = getDistance($lat, $lng, $val['lat'], $val['lng']);
        }
        $this->assign('tuans', $tuans);
