...
class CouponAction extends CommonAction {
    ...
    private $create_fields = array('coupon_id','shop_id','keyword_id','keyword','title','intro','photo','stime','ltime','use_tips','end_tips','end_photo','num','max_count','down_count','use_count','views','follower_condtion','clientip','dateline');

    private $edit_fields = array('coupon_id','shop_id','keyword_id','keyword','title','intro','photo','stime','ltime','use_tips','end_tips','end_photo','num','max_count','down_count','use_count','views','follower_condtion','clientip','dateline');
	...
	public function create()
	{
            ...
            if ($this->isPost()) {
                ...
                if ($id = $obj->add($data)) {
                    $url = U('weixin/coupon/detail', array('coupon_id' => $id));
                    $url = __HOST__ . $url;
    ...
    public function edit($coupon_id = null) {
        if ($coupon_id = (int) $coupon_id) {
            ...
            if ($this->isPost()) {
                ...
                $map['keyword'] = $data['keyword'];
                if($k = $objk->where($map)->select()&&$k['coupon_id'] == $coupon_id){
                    $this->baoError('该关键字已经被使用，请修改关键字');
                }else{
                    if($detail['keyword_id']){
                        ...
                        $url = U('weixin/coupon/detail', array('coupon_id' => $coupon_id));
                        $url = __HOST__ . $url;
                        $data1['url'] = $url;
                        $objk->save($data1);
                    }else{
        ...
        if($sn_id = (int)$sn_id){
			...
			if(!$detail = $obj->find($sn_id)){
				...
				$data['sn_id'] = $sn_id;
                                ...
                                if($obj->save($data)){
                                    if($detail['is_use'] == 1){
                                        D('Weixincoupon')->updateCount($detail['coupon_id'],'use_count',-1);
                                    }else{
                                        D('Weixincoupon')->updateCount($detail['coupon_id'],'use_count',1);
                                    }
