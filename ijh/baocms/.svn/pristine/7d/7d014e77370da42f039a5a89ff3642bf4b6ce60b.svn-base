...
class EleorderAction extends CommonAction {
    ...
    public function jqprint($order_id){
        if ( (!$order = D('EleOrder')->find($order_id)) || $order['closed'] == 1 ) {
            $this->baoError('订单不存在或已被删除！');
        }elseif ( (!$shop = D('Shop')->find($order['shop_id'])) || $shop['closed'] == 1 || $shop['audit'] == 0) {
            $this->baoError('商家不存在或已被删除');
        }elseif ($shop['user_id'] != $this->uid) {
            $this->baoError('请勿操作他人订单');
        }else{
    		//$shopsetting = D('Shopsetting')->where('shop_id='.$order['shop_id'])->find();
    ...
    public function jq_print($order_id=0){
        if(IS_AJAX){   
            if (!$order_id = (int) $order_id) {
                $this->ajaxReturn(array('status'=>'error','msg'=>'未选择的订单ID'));
            }elseif ( (!$order = D('EleOrder')->find($order_id)) || $order['closed'] == 1) {
                $this->ajaxReturn(array('status'=>'error','msg'=>'订单不存在或已被删除'));
            }elseif ($order['shop_id'] != $this->shop_id) {
                $this->ajaxReturn(array('status'=>'error','msg'=>'请勿操作其他商家的订单'));
            }else{
                $num = (int)$this->_param('num');
                if($arr = D('Eleorder')->order_print($order, $num)){
    ...
    public function queren($order_id) {
        ...
        $dorder = D('DeliveryOrder')->where(array('type_order_id'=>$order_id))->find();
        D('DeliveryOrder')->save(array('order_id'=>dorder['order_id'],'closed'=>0));
        $setting = D('Shopsetting')->where(array('shop_id'=>$this->shop_id))->find();
