...
class EleorderModel extends CommonModel {
    ...
    public function confirm($order_id,$auto=1,$shop_id,$num=1){ 
        if(is_array($order_id)){
            foreach($order_id as $id){
                ...
                D('DeliveryOrder')->save(array('order_id'=>$id,'status'=>2));
                if($auto == 1){
    ...
    public function order_print($order, $num=1){
        $shop = D('Shop')->find($order['shop_id']);
        if ($orderp = D('EleOrderProduct')->where('order_id ='.$order['order_id'])->select()) {
            $ids = array();
            foreach($orderp as $k => $v){
                $ids[$v['product_id']] = $v['product_id'];
            }
            if ($ep = D('EleProduct')->where(array('product_id'=>array('in',$ids)))->select()) {
                $ep2 = array();
                foreach($ep as $k => $v){
                    $ep2[$v['product_id']] = $v;
                }
            ...
            $setting = D('Shopsetting')->where(array('set_id'=>$shop['jqprint_id']))->find();
            //dump($setting);die;
        ...
        //return array('content'=>$content,'apiKey'=>$apiKey,'mKey'=>$mKey,'partner'=>$partner,'machine_code'=>$machine_code);
