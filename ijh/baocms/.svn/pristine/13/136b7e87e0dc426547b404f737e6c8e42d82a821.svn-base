    ...
    */
	...
	public function admin_wechat_client($shop_id=0)
    {
        ...
        if($client === null) {
            ...
            $client = new WechatClient($CONFIG['weixin']['appid'], $CONFIG['weixin']['appsecret']);
        }
        return $client;
    }
    ...
    public function weixinmenu($data,$shop_id = 0 ) {
        ...
        $ret = $this->curl->post($url, $datastr);
        $result = json_decode(ret, true);
        baolog('weixin.menu.mdl', array($data, $datas, $datastr, $url, $ret, $result));
        if ($result['errcode'] != 0) {
            return false;
        }
        return true;
