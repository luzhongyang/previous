...
class ActivitysignAction extends CommonAction{
    ...
    public  function index(){
       ...
       $shopids = array('in',D('Shop')->getCityShop($this->_fenzhan['city_id']));
       $activitys = D('Activity')->where(array('shop_id'=>$shopids))->select();
       $activityids = array();
       foreach($activitys as $k=>$v){
           $activityids[]=$v['activity_id'];
       }
       $activityids = implode(',',$activityids);
       $map['activity_id'] = array('in',$activityids); 
       ...
       if($activity_id){
           ...
           if($activity['shop_id'] != $this->_fenzhan['city_id']) {
