...
class MarketAction extends CommonAction {
    ...
    public function index() {
        ...
        $areas = D('Area')->where(array('city_id'=>$this->_fenzhan['city_id']))->select();
        foreach($areas as $k=>$v){
            $areaids[]=$v['area_id'];
        }
        $areaids = implode(',',$areaids);
        $map['area_id'] = array('in',$areaids);
    ...
    public function create() {
        if ($this->isPost()) {
            ...
            $areaids = implode(',', $areaids);
            $businesss = D('Business')->where(array('area_id'=>array('in',$areaids)))->select();
            
            $this->assign('fenzhan',$this->_fenzhan);
            $this->assign('areas', $areas);
            $this->assign('business', $businesss);
    ...
    private function createCheck() {
        ...
        if(!$area = D('area')->find($data['area_id'])){
            $this->baoError('区域不存在或已删除');
        }elseif ($area['city_id'] != $this->_fenzhan['city_id']){
            $this->baoError('区域必须属于当前城市');
        }
    ...
    public function edit($market_id = 0) {
        if ($market_id = (int) $market_id) {
            ...
            if ($this->isPost()) {
                ...
                $areaids = implode(',', $areaids);
                $businesss = D('Business')->where(array('area_id'=>array('in',$areaids)))->select();
                $this->assign('business', D('Business')->where(array('area_id'=>$detail['area_id']))->select());
                $this->assign('fenzhan',$this->_fenzhan);
    ...
    public function checkcity($market_id){
        $market = D('Market')->find($market_id);
        if(!$area = D('area')->find($market['area_id'])){
            $this->baoError('市场区域错误');
        }elseif ($area['city_id'] != $this->_fenzhan['city_id']){
            $this->baoError('市场必须属于当前城市');
        }
        return true;
    }
