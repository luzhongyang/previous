...
class ShopAction extends CommonAction {
    ...
    public function breaks($shop_id) { //优惠买单
        ...
        $shop_id = (int) $shop_id;
        if (!$shop_id) {
            $this->ajaxReturn(array('status'=>'error','msg'=>'该商家没有设置买单优惠'));
        } elseif (!$detail = D('Shopyouhui')->where(array('shop_id'=>$shop_id,'is_open'=>1))->find()) {
            $this->ajaxReturn(array('status'=>'error','msg'=>'该商家没有设置买单优惠或已关闭'));
        }
        if ($this->isPost()) {
            $amount = floatval($_POST['amount']);
            if(empty($amount)){
                $this->ajaxReturn(array('status'=>'error','msg'=>'消费金额不能为空'));
            }
            ...
            if ($order_id = D('Breaksorder')->add($data)) {
                $this->ajaxReturn(array('status'=>'success','msg'=>'下单成功!','order_id'=>$order_id,'amount'=>$amount,'url'=> U('shop/breakspay', array('order_id' => $order_id))));
            } else {
