...
class ShopnewsAction extends CommonAction {
    ...
    private function createCheck() {
        ...
        $map = array('shop_id' => $data['shop_id']);
        $city_id = D('Shop')->where($map)->getField('city_id');
    ...
    public function delete($news_id = 0) {
        if (is_numeric($news_id) && ($news_id = (int) $news_id)) {
            $obj = D('Shopnews');
            $shopnews = $obj->find($news_id);
            
            /* if($shopnews['city_id'] != $this->_fenzhan['city_id']){
                $this->baoError('数据不属于当前城市');
            } */
            
            $shop = D('Shop')->find($shopnews['shop_id']);
            if($shop['city_id'] != $this->_fenzhan['city_id']){
                $this->baoError('商家不属于当前城市');
            }
            $obj->delete($news_id);
