...
class LifeAction extends CommonAction {
    ...
    public function index() {
		...
		if ($select1 = (int) $this->_param('s1')){
			$map['select1'] = $select1;
            $this->assign('select1', $select1);
		}
		if ($select2 = (int) $this->_param('s2')){
			$map['select2'] = $select2;
            $this->assign('select2', $select2);
		}
		if ($select3 = (int) $this->_param('s3')){
			$map['select3'] = $select3;
            $this->assign('select3', $select3);
		}
		if ($select4 = (int) $this->_param('s4')){
			$map['select4'] = $select4;
            $this->assign('select4', $select4);
		}
		if ($select5 = (int) $this->_param('s5')){
			$map['select5'] = $select5;
            $this->assign('select5', $select5);
		}
    ...
    public function create($cat) {
        ...
        if ($this->isPost()) {
            ...
            if($this->_CONFIG['site']['lifelimit']>0){
                $s_today = strtotime(TODAY);
                $l_today = $s_today + 86399;
                if($this->_CONFIG['site']['lifelimit']<$count = D('Life')->where(array('create_time'=>array('between',$s_today.','.$l_today)))->count()){
                    $this->baoError('每天只能发布'.$this->_CONFIG['site']['lifelimit'].'条信息');
                }
            }
