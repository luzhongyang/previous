...
class alipay {
    ...
    public function getCode($logs, $setting) {
        ...
        if($real_method != 2){
            $parameter['receive_name']      = '网站用户';
            $parameter['receive_address']   = '北京市东城区东直门大街000号';
            $parameter['receive_zip']   = 100000;
            $parameter['receive_phone'] = '010-12345678';
            $parameter['receive_mobile'] = '15000000000';
        }
        if($logs['back_url']){
            $parameter['return_url'] = __HOST__ . U( 'pchome/payment/respond', array('code' => 'alipay','back_url'=>urlencode($logs['back_url']))); 
        }
