    ...
    */
    public function detail($product_id)
    {
        if(!$product_id = (int)$product_id){
            ...
            //商家评价
            $items = K::M('weidian/comment')->items(array('shop_id'=>$this->shop_id,'closed'=>0),array('comment_id'=>'desc'));
    ...
    public function order_create(){
        ...
        if(!$pei_type = $this->GP('pei_type')){
            ...
            if(!$addr['contact'] = $this->GP('contact')) {
                $this->msgbox->add('提货人不正确',215)->response();
            }else if(!$addr['mobile'] = $this->GP('mobile')) {
                $this->msgbox->add('手机号码不正确',215)->response();
            }else if(!$addr['addr'] = $this->GP('addr')) {
                $this->msgbox->add('提货地址不正确',215)->response();
            }else if(!$ziti_time){
                $this->msgbox->add('提货时间不能为空',214)->response();
            }else if($pei_time&&$pei_time < __TIME){
                $this->msgbox->add('提货时间不正确',215)->response();
            }
            $pei_type = 3; // order表pei_type = 3是自提单
        }elseif($pei_type == 1){
        ...
        if($order_id = K::M('order/order')->create($data,true)){  //jh_order
            ...
            $this->msgbox->set_data('online_pay',$online_pay);
        }else{
    ...
    // 自提订单选择商家提货点
    public function zitipoint()
    {
        $items = $new_items = $city_names = $city_ids = array();
        $city = K::M('data/city')->select();
        if($items = K::M('shop/ziti')->select(array('closed'=>0,'shop_id'=>WEIDIAN_SHOP_ID))) {
            foreach($items as $k=>$v) {
                foreach($city as $kk=>$vv) {
                    if($v['data_city_id'] == $vv['city_id']) {
                        $new_items[$kk] = $vv;
                    }
                }
            }
        ...
        ksort($new_items);
        $this->pagedata['city_list']= $new_items;
        $this->tmpl = 'weidian/product/zitipoint.html';
    }

    // 根据选择的城市名称获取商户在该城市自提点
    public function ajaxpoint()
    {
        $items = K::M('shop/ziti')->select(array('closed'=>0,'shop_id'=>WEIDIAN_SHOP_ID));
        $this->msgbox->add('success');
