...
//        }
        ...
        $filter['sku'] = '>:0';
        $filter['ltime'] = '>:' . time();
        if($items = K::M('shop/coupon')->items($filter, array('coupon_id' => 'desc'), null, null, $count)){
    ...
    public function loaditems($page=1)
    {
        ...
        $page = max((int)$page, 1);
        $limit = 10;
        if(!$items = K::M('member/coupon')->items($filter,array('coupon_id'=>'desc'),$page, $limit, $count)){
            $items = array();
        }else{
            $shop_ids = array();
            foreach($items as $k => $v){
