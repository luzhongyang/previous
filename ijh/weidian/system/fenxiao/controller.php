...
class Ctl extends Factory
{
    ...
    {
        if(!preg_match("/fx(\d+)/i", $this->request['host'], $m)){
            $this->error(404);
        }
        define('FX_SID',$m[1]);
        
        if(!$this->FENXIAO = K::M('fenxiao/fenxiao')->detail(FX_SID)){
            $this->error(404);
        }else if(empty($this->FENXIAO['status']) && $this->FENXIAO['uid'] != $this->uid){
            $this->msgbox->add('店铺审核中', 211)->response();
        }else{
            $this->FENXIAO['url'] = $this->request['host'];
            $total = array();
            $week_time = __TIME-86400*7;
            $total['product'] = K::M('weidian/product')->count(array('shop_id'=>$this->FENXIAO['shop_id'],'is_onsale'=>1,'closed'=>0,'type'=>'default','is_fenxiao'=>1));
            $total['new_product'] = K::M('weidian/product')->count(array('shop_id'=>$this->FENXIAO['shop_id'],'is_onsale'=>1,'closed'=>0,'type'=>'default','is_fenxiao'=>1,'dateline'=>'>:'.$week_time));
            $total['sale'] = K::M('weidian/product')->total_sale($this->FENXIAO['shop_id']);
    ...
    protected function _init_pagedata()
    {
        ...
        $this->pagedata['FENXIAO'] = $this->FENXIAO;
