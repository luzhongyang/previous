...
class Ctl_Client_Passport extends Ctl
{
    ...
    {
        if(!$wx_openid = $params['wx_openid']){
            $this->msgbox->add('缺少微信OPENID', 211);
        }else if($member = K::M('member/member')->member($wx_openid, 'wx_openid')){
            if($this->auth->manager($member['uid'])){
                $data = $this->filter_fields('uid,nickname,face,mobile', $member);
                $this->tuisongSet($params['register_id'], $data['uid']);
                $this->msgbox->add('success');
                $data['token'] = $this->auth->token;
                $data['wxtype'] = 'wxlogin';
                $data['wx_openid'] = $wx_openid;
                $member['tags']  = $this->getTag($data['uid']);
    ...
    public function wxbind($params)
    {
        ...
        if(!$wx_unionid = $params['wx_unionid']){
            ...
            K::M('member/member')->update($member['uid'], array('wx_openid'=>$params['wx_openid'], 'wx_unionid'=>$wx_unionid));
            if($this->auth->manager($member['uid'])){
                $data = $this->filter_fields('uid,nickname,face,mobile', $member);
                $data['token'] = $this->auth->token;
                $this->tuisongSet($params['register_id'], $data['uid']);
                $data['tags'] = $this->getTag($data['uid']);
                $this->msgbox->set_data('data', $data);
            }
        }else{
            $data = array('mobile'=>$mobile, 'wx_openid'=>$wx_unionid);
    ...
    /* 推送设置
     * @param $register_id 设备注册ID
     * @param $uid 用户ID
     */
    protected function tuisongSet($register_id = null, $uid = null)
    {
        if(empty($register_id) || empty($uid)) return false;
        $model = K::M('tuisong/tuisong');
        if($model->detail($uid)){
            $model->update($uid, array('register_id'=>$register_id));
        }else{
            $data = array(
                'uid'         => $uid,
    ...
    /* 用户所在推送组*/
    public function getTag($uid = null)
    {
        if(empty($uid)){
            return false;
        }
        $detail = K::M('tuisong/tuisong')->detail($uid);
