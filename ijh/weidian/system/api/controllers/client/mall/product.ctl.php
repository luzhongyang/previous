...
class Ctl_Client_Mall_Product extends Ctl
{
    ...
    public function index($params)
    {
        ...
        if($items = K::M('mall/product')->items(array('closed'=>0), null, 1, 10)){
            foreach($items as $k=>$v){
                $items[$k] = $this->filter_fields($this->_allow_fields, $v);
            }
        }
        $this->msgbox->set_data('data', array('items'=>array_values($items), 'banner_list'=>array_values($banner_list), 'nav_list'=>array_values($nav_list)));
    }
