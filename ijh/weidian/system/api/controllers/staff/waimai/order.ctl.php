...
class Ctl_Staff_Waimai_Order extends Ctl_Staff
{
    ...
    public function qiang($params)
    {
        ...
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff', 'log'=>sprintf(L('骑手(%s)准备为您配送'), $this->staff['name']), 'status'=>'2');
    ...
    public function pei($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff', 'log'=>sprintf(L('骑手(%s)开始为您配送'), $this->staff['name']), 'status'=>'4');
            K::M('order/log')->create($log);
            //通知会员
