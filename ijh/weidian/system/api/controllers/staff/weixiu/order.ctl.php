...
class Ctl_Staff_Weixiu_Order extends Ctl_Staff
{
...
o_lat,contact,mobile,addr,house,lng,lat,day,intro,order_from,dateline,cui_time,comment_status,jd_time,pay_code,pay_time,cate_id,cate_title,jiesuan_price,danbao_amount,order_status_label,order_status_warning,juli_label';
    ...
    public function index($params)
    {
        ...
        //查询条件
        $filter = $wait_filter = array('from'=>'weixiu', 'pay_status'=>'>:0', 'closed'=>0);
        ...
        if(in_array($params['status'], array(0,1,2,3))){
            switch ($params['status']){
                case 3: //已完成
                    $filter['order_status'] = array(4,5,6,7,8); // 44服务完成 8订单完成
                    $filter['pay_status'] = array(0,1);  //因为已完成中包含设置结算价格的订单
                ...
                case 1: //待接单                    
                    ...
                    //技能相关的新订单
                    if($attr_items = K::M("weixiu/attr")->items(array('staff_id'=>$this->staff_id))) {
                        $cate_ids = array();
                        foreach($attr_items as $k=>$v) {
                            $cate_ids[] = $v['cate_id'];
                        }
                        $filter['weixiu']['cate_id'] = $cate_ids;
                    }                    
                    $orderby = array('lasttime'=>'ASC');
                    break;
                default : 
        ...
        $limit = 10;
        if($orders = K::M('weixiu/order')->order_items($filter, $orderby, $page, $limit, $count)){
    ...
    public function detail($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            //voice_id,order_id,voice,voice_time,dateline
    ...
    public function qiang($params) 
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            //记录订单日志
            $log = array('order_id'=>$order_id, 'from'=>'staff','log'=>sprintf('已接单由师傅(%s)提供服务', $this->staff['name']), 'type'=>'2');
            K::M('order/log')->create($log);
            //增加订单统计
            K::M('staff/staff')->update_count($this->staff_id, 'orders', 1);
            //通知用户
            K::M('order/order')->send_member("师傅已接单", $log['log'], $order);
    ...
    public function cancel($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            if(K::M('order/order')->update($order_id, $a)){
                $log = array('order_id'=>$order_id, 'status'=>$a['order_status'], 'log'=>'师傅取消原因：'.$reason, 'from'=>'staff', 'dateline'=>__TIME);
                K::M('order/log')->create($log);
                //通知会员
                K::M('order/order')->send_member("师傅取消了订单", $log['log'], $order);
                $this->msgbox->add('SUCCESS');
            }
        }
    }
    ...
    public function setprice($params)
    {
        ...
        }else if(!K::M('weixiu/order')->set_price($order_id, $jiesuan_price, 'staff')){
            $this->msgbox->add('设置结算价格失败', 216);
        }else{
            $log = array('order_id'=>$order_id, 'status'=>5, 'log'=>sprintf('师傅设置了结算价格：￥%s', $jiesuan_price), 'from'=>'staff');
            K::M('order/log')->create($log);
            //通知会员
            K::M('order/order')->send_member("师傅设置了结算价格", $log['log'], $order);
            $this->msgbox->set_data('data', array('order_id'=>$order_id));
        }        
    }
    ...
    public function startwork($params)
    {
        if(!$order_id = (int)$params['order_id']){
            ...
            $log = array('order_id'=>$order_id, 'status'=>3, 'log'=>'师傅已经开始服务', 'from'=>'staff');
            K::M('order/log')->create($log);
            //通知会员
            K::M('order/order')->send_member("师傅已经开始服务", $log['log'], $order);
            $this->msgbox->set_data('data', array('order_id'=>$order_id));
        }
    }
    ...
    public function finshed($params)
    {
        ...
        if(!$order_id = (int)$params['order_id']){
            ...
            $log = array('order_id'=>$order_id, 'status'=>4, 'log'=>'师傅已经完成服务', 'from'=>'staff');
            K::M('order/log')->create($log);
            //通知会员
            K::M('order/order')->send_member("师傅已经完成服务", $log['log'], $order);
            $this->msgbox->set_data('data', array('order_id'=>$order_id));
        }
    }
}