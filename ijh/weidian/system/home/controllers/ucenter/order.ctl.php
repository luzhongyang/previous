...
class Ctl_Ucenter_Order extends Ctl_Ucenter {
    ...
    public function items_all($page)
    {
        $type = 0;//防止翻页错误,展示所有订单
        $this->check_login();
...
//        $pager['page'] = $page = max((int)$params['page'], 1);
        ...
        $this->pagedata['orders'] = $items;

        $this->page_next_pre($page,$limit, $count);//临时翻页

        $this->tmpl = 'ucenter/order/items_all.html';
    }

    public function items_maidan($page)
    {
        $type = 0;
        $page = max((int)$page, 1);
        $this->check_login();
        ...
        $filter['from'] = 'maidan';
        $orders = K::M('order/order')->items($filter,array('order_id'=>'DESC'),$page,$limit,$count);
        foreach($orders as $k => $v){
            $o = K::M('maidan/order')->detail($v['order_id']);
            if($o){
                $orders[$k]['order'] = $o;
            }else{
                unset($orders[$k]);
    ...
    public function page_next_pre($page,$limit, $count,$from=null,$type=null)
    {
        $arr_page = array();
        $arr_page['from'] = $from;
        $arr_page['type'] = $type;
        $arr_page['no_next'] = 0;
        $arr_page['pre'] = $page > 1 ? $page - 1 : 1;
        $max_page = ceil($count/$limit);
        $arr_page['next'] = $page + 1;
        if($arr_page['next']>$max_page){
            $arr_page['no_next'] = 1;
        }
        $this->pagedata['arr_page'] = $arr_page;
    }

    public function items($from, $type, $page=1)
    {
        if(!isset($type)){
            $type = 1;
        }
        ...
        if(!in_array($from, array('tuan','waimai','paotui','maidan','weixiu','house','other','mall'))){
            ...
            $this->page_next_pre($page,$limit, $count,$from, $type);//临时翻页
    ...
    public function comment_handle()  //外卖商家和普通商家评论通用
    {
        ...
        if($this->checksubmit()){
            ...
            }else if($order['from'] == 'waimai' && !$datas['pei_time']){
    ...
    public function chargeback($order_id)
    {
        ...
        if(!$order_id = (int)$order_id) {
            ...
            if(K::M('order/order')->cancel($order_id, $order, 'member')) {
                ...
                // 写入订单日志
                K::M('order/log')->create(array('order_id'=>$order['order_id'],'from'=>'member','log'=>'订单已取消','status'=>-1));
