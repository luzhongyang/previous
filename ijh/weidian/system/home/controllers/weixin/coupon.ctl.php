...
class Ctl_Weixin_Coupon extends Ctl_Weixin
{
	...
	public function preview($coupon_id)
	{
        if(!$coupon_id){
            $this->msgbox->add('未指定内容ID', 211);
        }else if(!$detail = K::M('weixin/coupon')->detail($coupon_id)){
            $this->msgbox->add('该商家优惠券不存在', 212);
        }else{
			...
			$list1 = K::M('weixin/couponsn')->items(array('shop_id'=>$detail['shop_id'],'openid'=>$openid,'is_use'=>1));
			$list2 = K::M('weixin/couponsn')->items(array('shop_id'=>$detail['shop_id'],'openid'=>$openid,'is_use'=>0));
