...
class Ctl_Mall extends Ctl
{
    ...
    public function index($cate_id, $new)
    {
        ...
        $this->pagedata['product_list'] = K::M('mall/product')->items($filter, $orderby, 1, 10);
        $this->pagedata['cate_list']= K::M('mall/cate')->tree();
    ...
    public function exchange(){
        ...
        if(!$my_addr = K::M('member/addr')->find(array('uid' => $this->uid, 'is_default' => 1))){
            $my_addr = K::M('member/addr')->find(array('uid' => $this->uid));
        }
