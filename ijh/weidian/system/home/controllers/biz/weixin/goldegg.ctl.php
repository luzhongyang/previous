...
class Ctl_Biz_Weixin_Goldegg extends Ctl_Biz_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'biz/weixin/goldegg/index.html';
    }
		...
		if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'shop_id'=>$this->shop_id))){
				if($goldegg_id = K::M('weixin/goldegg')->create($data)){
					...
					$this->msgbox->set_data('forward',$this->mklink('biz/weixin/goldegg/index'));
				} 
			}else{
           ...
           $this->tmpl = 'biz/weixin/goldegg/create.html';
        }
	}
	...
	public function edit($goldegg_id=null)
    {
        if(!($goldegg_id = (int)$goldegg_id) && !($goldegg_id = $this->GP('goldegg_id'))){
        	...
        	$this->pagedata['detail'] = $detail;
        	 $this->tmpl = 'biz/weixin/goldegg/edit.html';
        }
	}
	...
	public function sn($goldegg_id,$page_id)
	{
		if(!($goldegg_id = (int)$goldegg_id) && !($goldegg_id = $this->GP('goldegg_id'))){
			...
			$this->tmpl = 'biz/weixin/goldegg/sn.html';
		}
	} 
