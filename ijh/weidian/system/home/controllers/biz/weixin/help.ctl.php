...
class Ctl_Biz_Weixin_Help extends Ctl_Biz_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'biz/weixin/help/index.html';
    }
		...
		if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'shop_id'=>$this->shop_id))){
				if($help_id = K::M('weixin/help')->create($data)){
					...
					$this->msgbox->set_data('forward',$this->mklink('biz/weixin/help/index'));
				} 
			}else{
				$this->msgbox->add('该关键字已经被使用，请修改关键字', 212);
			}
        }else{
           $this->tmpl = 'biz/weixin/help/create.html';
        }
	}
	...
	public function edit($help_id=null)
    {
        if(!($help_id = (int)$help_id) && !($help_id = $this->GP('help_id'))){
        	...
        	$this->pagedata['detail'] = $detail;
        	 $this->tmpl = 'biz/weixin/help/edit.html';
        }
	}
	...
	public function sn($help_id,$page=1)
	{
		if(!($help_id = (int)$help_id) && !($help_id = $this->GP('help_id'))){
			...
			$this->tmpl = 'biz/weixin/help/sn.html';
		}
	} 
	...
	public function goods($help_id,$page_id)
	{
		if(!($help_id = (int)$help_id) && !($help_id = $this->GP('help_id'))){
			...
			$this->pagedata['detail'] = $detail;
			$this->pagedata['pager'] = $pager;
			$this->tmpl = 'biz/weixin/help/goods.html';
		}
	}
	...
	public function goodscreate($help_id)
    {
		if(!($help_id = (int)$help_id) && !($help_id = $this->GP('help_id'))){
            ...
            if($id = K::M('weixin/helpprize')->create($data)){
                $this->msgbox->add('添加内容成功');
				$this->msgbox->set_data('forward', $this->mklink('biz/weixin/help/goods',array($help_id)));
            } 
		}else{
		   $this->pagedata['help_id'] = $help_id;
		   $this->tmpl = 'biz/weixin/help/goodscreate.html';
		}
    }
	...
	public function goodsedit($id=null)
    {
        if(!($id = (int)$id) && !($id = $this->GP('id'))){
        	...
        	$this->pagedata['detail'] = $detail;
			$this->tmpl = 'biz/weixin/help/goodsedit.html';
        }
    }
	...
	public function snlist($sn_id,$page_id)
	{
        ...
        if(!$sn_id){
			...
			$this->pagedata['detail'] = $detail;
			$this->pagedata['pager'] = $pager;
			$this->tmpl = 'biz/weixin/help/snlist.html';
		}
	}
}