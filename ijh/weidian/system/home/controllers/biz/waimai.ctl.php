...
Import::C('biz/biz');
class Ctl_Biz_Waimai extends Ctl_Biz
{
    ...
    public function __construct($system)
    {
        ...
        if(!($waimai = K::M('waimai/waimai')->detail($this->shop_id)) || empty($waimai['audit'])){
            ...
            if($system->request['ctl'] != 'biz/waimai' || $system->request['act'] != 'index'){
                $this->msgbox->add('您需要开通外送店铺才能使用该功能', 311);
                $this->msgbox->set_data('forward', $this->mklink('biz/waimai:index'));
    ...
    public function index()
    {
        if($data = $this->checksubmit('data')){
            ...
            $this->tmpl = 'biz/waimai/index.html';
        }
    }
}