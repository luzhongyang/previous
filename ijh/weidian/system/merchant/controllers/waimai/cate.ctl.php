...
Import::C('waimai');
class Ctl_Waimai_Cate extends Ctl_Waimai
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('waimai/productcate')->items($filter, null, $page, $limit, $count)){
            $pager['count'] = $count;
            $pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink('merchant/waimai/cate/index', array('{page}')), array('SO'=>$SO));
        }
    ...
    public function create($parent_id=null)
    {
        if($data = $this->checksubmit('data')){
            if(!$data['title']) {
                $this->msgbox->add('请填写名称',210)->response();
            }else if(!$data['orderby']) {
                $this->msgbox->add('请填写排序',211)->response();
            }
            $data['shop_id'] = $this->shop_id;
            if($cate_id = K::M('waimai/productcate')->create($data)){
                $this->msgbox->add('添加内容成功');
                $this->msgbox->set_data('forward', $this->mklink('merchant/waimai/cate/index'));
            } 
        }else{
    ...
    public function edit($cate_id=null)
    {
        ...
        if(!($cate_id = (int)$cate_id) && !($cate_id = $this->GP('cate_id'))){
            ...
            if(K::M('waimai/productcate')->update($cate_id, $data)){
                ...
                $this->msgbox->set_data('forward',  $this->mklink('merchant/waimai/cate/edit',array('args'=>$cate_id)));
            }  
        }else{
            $this->pagedata['cates'] = K::M('waimai/productcate')->items(array('parent_id'=>0, 'shop_id'=>$this->shop_id));
            $this->pagedata['detail'] = $detail;
            $this->tmpl = 'merchant:waimai/cate/edit.html';
        }    
    }
