...
{
    protected $_default_request = array('ctl'=>'cashier','act'=>'index','type'=>'html','args'=>null);
    ...
    protected function _route($uri=null)
    {
        ...
        if(substr($request['ctl'], 0, 4) == 'biz/'){
            $request['ctl'] = substr($request['ctl'], 4);
        }
        switch($request['ctl']){
            case 'cashier':
                $request['ctl'] = 'cashier/index';
                break;
            case 'cashire/index':
                $request['ctl'] = 'cashire/index/index';
                break;
