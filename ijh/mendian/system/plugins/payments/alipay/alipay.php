...
class Payment_Alipay 
{
    ...
    public function qrcodepay($params)
    {
        ...
        if($qrPayResult->getTradeStatus() == 'SUCCESS'){
            ...
            $trade = array('code'=>'alipay', 'trade_no'=>$params['trade_no'], 'qrcode'=>$response->qr_code, 'amount'=>$params['amount']);
            return $trade;
        }
        return false;
    }
    ...
    {
        if(defined('IN_MOBILE')){
            return $this->build_wap_url($params);
    ...
    public function build_wap_url($params)
    {
        Import::L('alipay/AopSdk.php');
        $config = $this->getAppConfig();
        $AopClient = new AopClient();
        $AopClient->appId = $this->config['alipay_appid'];
        $AopClient->rsaPrivateKey = $this->config['alipay_rsa_private'];
        $AopClient->alipayrsaPublicKey = $this->config['alipay_rsa_public'];
        ...
        $request->setBizContent('{'.
            ...
            '"timeout_express":"90m"'.
            '"total_amount":"'.sprintf("%01.2f", $params['amount']).'"'.
    ...
    public function refund($params, &$msg='SUCCESS')
    {
        ...
        $bizContent['refund_reason'] = $params['refund_reason'] ? $params['refund_reason'] : "正常退款";
    ...
    public function query($params, $msg='SUCCESS')
    {
    ...
    public function return_verify()
    {
        ...
        if(empty($_GET)){   //判断GET来的数组是否为空
            ...
            $this->_logs($log);
            if ($verify_sign){
                $trade = array('code'=>'alipay','trade_no'=>$notify['out_trade_no'], 'pay_trade_no'=>$notify['trade_no'], 'trade_status'=>$_GET['trade_status'], 'amount'=>$notify['total_fee'], 'extra_param'=>$this->_decode_params($notify['extra_common_param']));
                $trade['amount'] = $notify['total_amount'] ? $notify['total_amount'] : $notify['total_fee'];
                return $trade;
            }
            return false;
        }else{
    ...
    public function notify_verify()
    {
        ...
        if(empty($_POST)){//判断POST来的数组是否为空
            ...
            return false;
        }else if(!defined('IN_APP') && defined('IN_MOBILE')){   
            ...
            $this->_logs($log);
            if ($verify_sign){                  
                $trade = array('code'=>'alipay','trade_no'=>$notify['out_trade_no'], 'pay_trade_no'=>$notify['trade_no'], 'trade_status'=>$_POST['trade_status'], 'amount'=>$notify['total_amount'], 'extra_param'=>$this->_decode_params($notify['extra_common_param']));
                $trade['amount'] = $notify['total_amount'] ? $notify['total_amount'] : $notify['total_fee'];
                return $trade;
            }             
            return false;
