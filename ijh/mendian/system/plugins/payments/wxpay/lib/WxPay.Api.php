...
class WxPayApi
{
	...
	{		
		...
		if($data){
			...
			$error = curl_error($ch);
			curl_close($ch);
			$a = file_get_contents(WxPayConfig::$SSLCERT_PATH);
			$b = file_get_contents(WxPayConfig::$SSLKEY_PATH);
			K::M('system/logs')->log('wxpay.api.postxmlcurl', array($xml, $data, WxPayConfig::$SSLCERT_PATH, WxPayConfig::$SSLKEY_PATH, $a, $b, $errno, $error));
			throw new WxPayException("curl出错，错误码:$errno,$error");
		}
	}
