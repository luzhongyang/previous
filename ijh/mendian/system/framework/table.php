 ...
 * $Id: table.php 9343 2015-03-24 07:07:00Z youyi $
 */
...
class Mdl_Table extends Model
{
   ...
   public function select($filter=array(),$orderby=null,$l)
    ...
    public function find($filter=array(),$orderby=null)
    {
       $where = $this->where($filter);
       $orderby = $this->order($orderby);
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE $where $orderby";
    ...
    public function fetch_fields($table=null)
    {
        $data = false;
                    ...
                    if(!in_array($k,$cols)){
                        ...
                        if(isset($filter[$k])){
                            if(preg_match('/^(\d+)(d|w|m)$/i', $filter['dateline'], $m)){
                                ...
                                $v = ">:{$time}";
                            }else if(is_numeric($filter['dateline']) && $filter['dateline'] < 31536000){
    ...
    public static function _quote($val, $noarray=false)
    {
        if (is_string($val))
            ...
            foreach($schemas as $k=>$v){
                ...
                if(isset($data[$k])){
                    switch(strtolower($v['type'])){
                        case 'int': case 'member' : case 'staff': case 'shop':
