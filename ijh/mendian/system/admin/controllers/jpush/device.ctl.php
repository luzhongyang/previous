...
class Ctl_Jpush_Device extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('jpush/device')->items($filter, null, $page, $limit, $count)){
            ...
            $uids = $staff_ids = $shop_ids = $cashier_staff_ids = $cashier_shop_ids = array();
            foreach($items as $v){
                if($v['from'] == 'member'){
                    $uids[$v['uid']] = $v['uid'];
                }else if($v['from'] == 'cashier'){
                    $cashier_staff_ids[$v['staff_id']] = $v['staff_id'];
                    $shop_ids[$v['shop_id']] = $v['shop_id'];
                }else if($v['from'] == 'shop'){
                    $shop_ids[$v['shop_id']] = $v['shop_id'];
                }else if($v['from'] == 'staff'){
            ...
            if($cashier_staff_ids){
                $this->pagedata['cashier_staff_list'] = K::M('cashier/staff')->items_by_ids($cashier_staff_ids);
            }
        }
    ...
    public function push($device_id=null)
    {
        if($data = $this->checksubmit('data')){
            ...
            if($device = K::M('jpush/device')->detail($device_id)){
                ...
                if($device['from'] == 'member'){
                    $this->pagedata['member'] = K::M('member/member')->detail($device['uid']);
                }else if($device['from'] == 'shop'){
                    $this->pagedata['shop'] = K::M('shop/shop')->detail($device['shop_id']);
                }else if($device['from'] == 'cashier'){
                    $this->pagedata['cashier_staff'] = K::M('cashier/staff')->detail($device['staff_id']);
                }else if($device['from'] == 'staff'){
                    $this->pagedata['staff'] = K::M('staff/staff')->detail($device['staff_id']);
                }
