...
class Ctl_Cashier_Cate extends Ctl
{
    ...
    public function index($page=1)
    {
        ...
        if($items = K::M('cashier/product/cate')->items($filter, null, $page, $limit, $count)){
        	$pager['count'] = $count;
        	$pager['pagebar'] = $this->mkpage($count, $limit, $page, $this->mklink(null, array('{page}')), array('SO'=>$SO));
        }
        $this->pagedata['items'] = $items;
        $this->pagedata['pager'] = $pager;
        $this->tmpl = 'admin:cashier/product/cate/items.html';
    }
    ...
    public function create()
    {
        if($data = $this->checksubmit('data')){
            ...
            if($cate_id = K::M('cashier/product/cate')->create($data)){
                ...
                $this->msgbox->set_data('forward', '?cashier/cate-index.html');
            } 
        }else{
           $this->tmpl = 'admin:cashier/product/cate/create.html';
        }
    }
    public function edit($cate_id=null)
    {
        if(!($cate_id = (int)$cate_id) && !($cate_id = $this->GP('cate_id'))){
        	...
        	$this->tmpl = 'admin:cashier/product/cate/edit.html';
        }
    }
    ...
    public function delete($cate_id=null)
    {
        if($cate_id = (int)$cate_id){
            ...
            if(K::M('cashier/product/cate')->delete($ids)){
