...
{   
    ...
    protected $_cols = 'article_id,city_id,cate_id,from,page,title,thumb,desc,linkurl,views,favorites,allow_comment,comments,photos,hidden,orderby,audit,closed,dateline';
    ...
    public function detail($article_id, $closed=false)
    {
        ...
        if($detail = $this->db->GetRow($sql)){
            $cate = K::M('article/cate')->cate($detail['cate_id']);
    ...
    public function prev_item($article_id, $cate_id=0)
    {
        $where = '';
        if(!$article_id = (int)$article_id){
            return false;
        }else if($cate_id = (int)$cate_id){
            $where = "cate_id='{$cate_id}' AND ";
        }
    ...
    public function next_item($article_id, $cate_id=0)
    {
        $where = '';
        if(!$article_id = (int)$article_id){
            return false;
    ...
    protected function _format_row($row)
    {
        ...
        if($cate = K::M('article/cate')->cate($row['cate_id'])){
