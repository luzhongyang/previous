...
class Mdl_Payment_Log extends Mdl_Table
{
    ...
    protected $_cols    = 'log_id,uid,shop_id,from,type,payment,trade_no,order_id,amount,payed,payedip,payedtime,pay_trade_no,extra_pay,pay_level,payee,payee_info,clientip,dateline';
    ...
    public function log_by_order_id($order_id, $level=0)
    {
        ...
        $level = (int)$level;
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE order_id=$order_id AND pay_level=$level AND `type`=>'pay'";
    ...
    public function refund_by_order_id($order_id)
    {
        if(!is_numeric($order_id)){
            return false;
        }
        $sql = "SELECT * FROM ".$this->table($this->_table)." WHERE order_id=$order_id AND `type`=>'refund' ORDER BY log_id DESC";
        if($row = $this->db->GetRow($sql)){
            $row = $this->_format_row($row);
        }
        return $row;        
    ...
    {
        $row['payee_info'] = json_decode($row['payee_info']);
        return parent::_format_row($row); // TODO: Change the autogenerated stub
    }
    ...
    protected function _check($data, $log_id=null)
    {
        ...
        if(is_array($data['payee_info'])){
            $data['payee_info'] = json_encode($data['payee_info']);
        }
