 ...
 */
class Mdl_Magic_Verify extends Mdl_Table
{
    function output($mobile = '1', $W = 120, $H = 35)
    {
        $S = $this->reset($mobile);
        $L = strlen($S);
        $char_X = 6;
    ...
    public function check($code, $mobile)
    {
        $session = K::M('system/session')->start();
//        if($vcode = $session->get('VIMGCODE')){
        //session机制出错,采用cache机制
        if($vcode = $this->cache->get('VIMGCODE' . $mobile)){
