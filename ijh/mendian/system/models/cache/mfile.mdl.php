...
class Mdl_Cache_MFile implements Cache_Interface
{
	...
	{
		$time = empty($ttl) ? 0 : (__CFG::TIME + $ttl);
		$hash = md5($key);
...
if(!defined("__CORE_DIR")){	exit("Access Denied");}if({time}===0 || __TIME<{time}){return {data};}return false;';
		...
		file_put_contents($this->cache_file($key), $content);
	}
	...
	{
		K::M('io/file')->remove($this->cache_file($key));
	}
    ...
    {
        return $this->cache_dir.'cache_'.md5($key).str_replace(array("\/", "\\"), array(".", "."), $key).'.php';
    }
}
