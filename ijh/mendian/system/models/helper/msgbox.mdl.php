...
class Mdl_Helper_MsgBox
{
    ...
    public function show($url='',$t='HTML', $timer=3)
    {
        ...
        if('HTML' == $t){
            ...
            if(defined('IN_APP') && IN_APP && defined('__API_ENDATA') && __API_ENDATA){
                echo K::M('secure/mcrypt')->encode($this->outputdata, __API_KEY);
            }else{
    ...
    {
        if(defined('IN_ADMIN')){
            $this->tmpl = 'admin:page/alert.html';
        }else{
            $this->tmpl = 'view:page/alert.html';
        }
        $this->show($url,'JS');
    }
