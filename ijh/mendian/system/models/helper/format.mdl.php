...
class Mdl_Helper_Format
{
    ...
    {
        if(!$longurl = K::M('verify/check')->url($longurl)){
            return false;
        }else if($ret = K::M('net/htpp')->get('http://api.weibo.com/short_url/shorten.json', array('source'=>'500366327', 'url_long'=>$longurl))){
            if($ret = json_decode($ret)){
                if($ret['urls']){
                    return $ret['urls']['url_short'];
                }
            }
        }
        return false;
