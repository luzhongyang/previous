...
class Mdl_Cashier_Auth extends Model
{
    public $staff_id = 0;
    public $name = '';
    public $staff = array();
    ...
    public function login($u, $p, $l=null, $ismd5=false, $keep=false)
    {
        ...
        if(!$staff = K::M('cashier/staff')->staff($u, $l)){
            ...
            $this->token = $token;
            $staff['token'] = $token;
            K::M('cashier/staff')->update($staff['staff_id'], array('lastlogin'=>__CFG::TIME, 'loginip'=>__IP), true);
            return $staff;
        }
        return false;       
    }
    ...
    public function manager($staff_id){
        ...
        if(!$staff = K::M('cashier/staff')->detail($staff_id)){
           return false;
        }else{
    ...
    public function update_passwd($pwd, $ismd5=true)
    {
        ...
        }else if(K::M('cashier/staff')->update($this->staff_id, array('passwd'=>$md5pwd))){
            $this->passwd = $md5pwd;
