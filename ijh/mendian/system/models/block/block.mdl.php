...
{   
    ...
    protected function _check($data, $block_id=null)
    {
        ...
        if(isset($data['config']) && is_array($data['config'])){
            ...
            if(isset($data['config']['cate_id']) && is_array($data['config']['cate_id'])){
                $cate_ids = array();
                foreach($data['config']['cate_id'] as $v){
                    if($v = (int)$v){
                        $cate_ids[] = $v;
                    }
                }
                $data['config']['cate_id'] = $cate_ids;
            }            
        ...
        if(!$block_items = $this->cache->get($cache_key, $block['ttl'])){
            ...
            if($block['type'] != 'only' && $limit > $count){
                ...
                if($block['from'] == 'article'){
                    ...
                    if($cate_ids = K::M('verify/check')->ids($block['config']['cate_id'])){
                        if(is_numeric($cate_ids)){
                ...
                if($bfilter = $this->_block_filter($block)){
                    unset($bfilter['group_id'], $bfilter['cate_id'], $bfilter['sale_tuijian'], $bfilter['sale_remai'], $bfilter['sale_youhui'], $bfilter['onpayment'], $bfilter['score']);
