...
class Ctl_Weixin_Packet extends Ctl_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'merchant:weixin/packet/index.html';
    }
		...
		if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'shop_id'=>$this->shop_id))){
				if($id = K::M('weixin/packet')->create($data)){
					...
					$this->msgbox->set_data('forward',$this->mklink('merchant/weixin/packet/index'));
				} 
			}else{
           ...
           $this->tmpl = 'merchant:weixin/packet/create.html';
        }
	}
	...
	public function edit($id=null)
    {
        if(!($id = (int)$id) && !($id = $this->GP('id'))){
        	...
        	$this->pagedata['detail'] = $detail;
        	 $this->tmpl = 'merchant:weixin/packet/edit.html';
        }
	}
	...
	public function sn($id,$page)
	{
		if(!($id = (int)$id) && !($id = $this->GP('id'))){
			...
			$this->tmpl = 'merchant:weixin/packet/sn.html';
		}
	} 
	...
	public function logs($id,$page_id)
	{
		if(!($id = (int)$id) && !($id = $this->GP('id'))){
			...
			$this->pagedata['detail'] = $detail;
			$this->pagedata['pager'] = $pager;
			$this->tmpl = 'merchant:weixin/packet/logs.html';
		}
	} 
