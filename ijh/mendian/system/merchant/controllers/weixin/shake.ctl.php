...
class Ctl_Weixin_Shake extends Ctl_Weixin
{
    ...
    public function index($page=1)
    {
        ...
        $this->tmpl = 'merchant:weixin/shake/index.html';
    }
		...
		if($data = $this->checksubmit('data')){
			...
			if(!$items = K::M('weixin/keyword')->items(array('keyword'=>$data['keyword'],'shop_id'=>$this->shop_id))){
				if($shake_id = K::M('weixin/shake')->create($data)){
					...
					$this->msgbox->set_data('forward', $this->mklink('merchant/weixin/shake/index'));
				} 
			}else{
				$this->msgbox->add('该关键字已经被使用，请修改关键字', 212);
			}
        }else{
           $this->tmpl = 'merchant:weixin/shake/create.html';
        }
	}
	...
	public function edit($shake_id=null)
    {
        if(!($shake_id = (int)$shake_id) && !($shake_id = $this->GP('shake_id'))){
        	...
        	$this->tmpl = 'merchant:weixin/shake/edit.html';
        }
	}
	...
	public function sn($shake_id,$page=1)
	{
		if(!($shake_id = (int)$shake_id) && !($shake_id = $this->GP('shake_id'))){
			...
			$this->tmpl = 'merchant:weixin/shake/sn.html';
		}
	} 
	...
	public function goods($shake_id,$page=1)
	{
		if(!($shake_id = (int)$shake_id) && !($shake_id = $this->GP('shake_id'))){
			...
			$this->pagedata['detail'] = $detail;
			$this->pagedata['pager'] = $pager;
			$this->tmpl = 'merchant:weixin/shake/goods.html';
		}
	}
	...
	public function goodscreate($shake_id)
    {
		if(!($shake_id = (int)$shake_id) && !($shake_id = $this->GP('shake_id'))){
            ...
            if($id = K::M('weixin/shakeprize')->create($data)){
                $this->msgbox->add('添加内容成功');
				$this->msgbox->set_data('forward',$this->mklink('merchant/weixin/shake/goods',array($shake_id)));
            } 
		}else{
		   $this->pagedata['shake_id'] = $shake_id;
		   $this->tmpl = 'merchant:weixin/shake/goodscreate.html';
		}
    }
	...
	public function goodsedit($id=null)
    {
        if(!($id = (int)$id) && !($id = $this->GP('id'))){
        	...
        	$this->pagedata['detail'] = $detail;
			$this->tmpl = 'merchant:weixin/shake/goodsedit.html';
        }
    }
