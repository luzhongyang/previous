...
define(function(require, exports) {
	...
	var _download = function(from,to,callback){
		$.ajax({
			url:'/index.php/chat/explorer/serverDownload?save_path='+to
				 +'&url='+urlEncode2(from),
			dataType:'json',
	...
	var _unzip = function(file,unzip_to,callback){
		$.ajax({
			url:'/index.php/chat/explorer/unzip?path_to='+urlEncode(unzip_to)
				+'&path='+urlEncode(file),
			success:function(data){
				if (typeof (callback) == 'function') callback(data);
			}
	...
	var _remove = function(param,callback){
		$.ajax({
			url: '/index.php/chat/explorer/pathDelete',
			type:'POST',
			dataType:'json',
			data:param,
			success:function(data){
