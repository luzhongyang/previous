...
function show_json_netbot($data,$result = 1,$msg=''){
	$result = array('result' => $result,'msg'=>$msg,'data' => $data);
	header("X-Powered-By: kodExplorer.");
	header('Content-Type: application/json; charset=utf-8');
	echo json_encode($result,JSON_UNESCAPED_UNICODE);
	exit;
...
//分片上传处理
function upload_chunk($fileInput, $path = './',$temp_path,$zz_name=""){
	$file = $_FILES[$fileInput];
	$chunk = isset($_REQUEST["chunk"]) ? intval($_REQUEST["chunk"]) : 0;
	$chunks = isset($_REQUEST["chunks"]) ? intval($_REQUEST["chunks"]) : 1;
	if (!isset($file)) show_json("上传文件为空",false);
	//$file_name = $file['name'];
  ...
  if($zz_name){
	...
	if ($chunks>1) {//并发上传，不一定有前后顺序
		$temp_file_pre = $temp_path.md5($temp_path.$file['name']).'.part';
		if (get_filesize($file['tmp_name']) ==0) {
			show_json("upload_success",false,'chunk_'.$chunk.' error!');
		}
       ...
       $save_path = $path.$file_name;     
			...
			return true;	
			//show_json("upload_success",true,$save_path);
		}else {
	...
	//正常上传
	$save_path = $path.$file_name; //自动重命名
...
function get_nb_internet($flags){
...
if (($flags & $INTERNET_CONNECTION_MODEM) == $INTERNET_CONNECTION_MODEM) return "在线：拨号上网";
if (($flags & $INTERNET_CONNECTION_LAN) == $INTERNET_CONNECTION_LAN) return "在线：通过局域网";
if (($flags & $INTERNET_CONNECTION_PROXY) == $INTERNET_CONNECTION_PROXY) return "在线：代理";
if (($flags & $INTERNET_CONNECTION_MODEM_BUSY) == $INTERNET_CONNECTION_MODEM_BUSY) return "MODEM被其他非INTERNET连接占用";
return "未知网络";
}