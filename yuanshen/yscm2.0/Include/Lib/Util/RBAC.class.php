...
class RBAC {
	...
	*/
	static public function getAccessList($authId){
		...
		$table['node']." as node ".
		"where user.id='{$authId}' and user.role_id=role.id and access.role_id=role.id and role.status=1 and access.node_id=node.id and node.level=1 and node.status>=0";
		...
		foreach($apps as $key=>$app) {
			...
			$table['access']." as access ,".
			$table['node']." as node ".
			"where user.id='{$authId}' and user.role_id=role.id and access.role_id=role.id and role.status=1 and access.node_id=node.id and node.level=2 and node.pid={$appId} and node.status>=0";
			...
			foreach($modules as $key=>$module) {
				...
				if('PUBLIC'== strtoupper($moduleName)) {
					...
					$table['access']." as access ,".
					$table['node']." as node ".
					"where user.id='{$authId}' and user.role_id=role.id and  access.role_id=role.id and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$moduleId} and node.status>=0";
					$rs =   $db->query($sql);
					foreach ($rs as $a){
		...
		/*修改$access数组中的第一级为项目名称*/
		$access1[strtoupper(APP_NAME)] = array();
		foreach ($access as $key => $value) {
			$access1[strtoupper(APP_NAME)]+=$value;
		}
	...
	static public function getModuleAccessList($authId,$module) {
		...
		$table['user']." as user,".
		$table['access']." as access ".
		"where user.id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and  access.module='{$module}' and access.status=1";
		$rs =  $db->query($sql);
		$access = array();
		foreach ($rs as $node){
			$access[]=$node['node_id'];
