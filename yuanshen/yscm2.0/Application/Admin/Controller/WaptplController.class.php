...
class WaptplController extends CommonController{	
	// 显示模板管理
	public function index(){
		...
		foreach($list_dir as $key=>$value){
			$list_dir[$key]['pathfile'] = url_repalce($value['path'],'desc').'|'.$value['filename'];
		}
		$_SESSION['tpl_jumpurl'] = 'Waptpl/index/id/'.url_repalce($dirpath,'desc');
		if($dirlast && $dirlast != '.'){
			$this->assign('dirlast',url_repalce($dirlast,'desc'));
		}		
	...
	// 编辑模板
	public function add(){
		$filename = url_repalce(str_replace('*','.',trim($_GET['id'])));
		if (empty($filename)) {
			$this->error('模板名称不能为空！');
		}
	...
	public function update(){
		...
		$this->success('恭喜您，模板更新成功！',U('waptpl/index'));
	}
	// 删除模板
	public function del(){
		$id = url_repalce(str_replace('*','.',trim($_GET['id'])));
		if (!substr(sprintf("%o",fileperms($file)),-3)){
			$this->error('无删除权限！');
		}
		@unlink($file);
		$this->success('删除文件成功！',U('waptpl/index'));
	}				
}
