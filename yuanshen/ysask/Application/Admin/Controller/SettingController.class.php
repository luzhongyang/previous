...
class SettingController extends CommonController{
	...
	public function site_update(){
		if (IS_POST) {
			...
			$Setting_config["site_user_defaultavatar"] = $_POST["site_user_defaultavatar"];
			//保存配置
			$config = "<?php\r\nif(!defined('THINK_PATH')) exit();\r\nreturn \$array = ".var_export($Setting_config, TRUE).";\r\n?>";
			file_put_contents("Data/Conf/config.ini.php",$config);
			//保存PC端风格配置
	...
	public function file_update(){
		if (IS_POST) {
			...
			//阿里云
			$Setting_config["aliyun_access_key"] = $_POST["aliyun_access_key"];
			$Setting_config["aliyun_secret_key"] = $_POST["aliyun_secret_key"];
			$Setting_config["aliyun_siteurl"] = $_POST["aliyun_siteurl"];
			$Setting_config["aliyun_sitename"] = $_POST["aliyun_sitename"];
			$Setting_config["aliyun_imgstyle1"] = $_POST["aliyun_imgstyle1"];
			$Setting_config["aliyun_imgstyle2"] = $_POST["aliyun_imgstyle2"];
			$Setting_config["aliyun_imgstyle3"] = $_POST["aliyun_imgstyle3"];
			$Setting_config["aliyun_imgstyle4"] = $_POST["aliyun_imgstyle4"];
	...
	public function userstrategy_update(){
		if (IS_POST) {
			require "Data/Conf/config.ini.php";
			$Setting_config = $array;
			if(!preg_match("/^\d*$/",I('post.experience_register'))) {
				$this->error('用户注册获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_register'))) {
				$this->error('用户注册获得金币值不正确');
			}else if(!preg_match("/^\d*$/",I('post.experience_login'))) {
				$this->error('用户登录获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_login'))) {
				$this->error('用户登录获得金币值不正确');
			}else if(!preg_match("/^\d*$/",I('post.experience_ask'))) {
				$this->error('用户提问获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_ask'))) {
				$this->error('用户提问获得金币值不正确');
			}else if(!preg_match("/^\d*$/",I('post.experience_answer'))) {
				$this->error('用户回答问题获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_answer'))) {
				$this->error('用户回答问题获得金币数不正确');
			}else if(!preg_match("/^\d*$/",I('post.experience_adopted'))) {
				$this->error('用户回答被采纳获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_adopted'))) {
				$this->error('用户回答被采纳获得金币值不正确');
			}else if(!preg_match("/^\d*$/",I('post.experience_article'))) {
				$this->error('用户撰写文章获得经验值不正确');
			}else if(!preg_match("/^\d*$/",I('post.coin_article'))) {
				$this->error('用户撰写文章获得金币数不正确');
			}else {
				$Setting_config["experience_register"] = $_POST["experience_register"];
				$Setting_config["coin_register"] = $_POST["coin_register"];
				$Setting_config["experience_login"] = $_POST["experience_login"];
				$Setting_config["coin_login"] = $_POST["coin_login"];
				$Setting_config["experience_ask"] = $_POST["experience_ask"];
				$Setting_config["coin_ask"] = $_POST["coin_ask"];
				$Setting_config["experience_answer"] = $_POST["experience_answer"];
				$Setting_config["coin_answer"] = $_POST["coin_answer"];
				$Setting_config["experience_adopted"] = $_POST["experience_adopted"];
				$Setting_config["coin_adopted"] = $_POST["coin_adopted"];
				$Setting_config["experience_article"] = $_POST["experience_article"];
				$Setting_config["coin_article"] = $_POST["coin_article"];
	...
	public function access_update(){
		if (IS_POST) {
			require "Data/Conf/config.ini.php";
			$Setting_config = $array;
			$Setting_config["signup_count"] = intval($_POST["signup_count"]);
			$Setting_config["signup_time"] = intval($_POST["signup_time"]);
			$Setting_config["denyip"] = $_POST["denyip"];
			$Setting_config["mobile_count"] = intval($_POST["mobile_count"]);
			$Setting_config["mobile_time"] = intval($_POST["mobile_time"]);
			$config = "<?php\r\nif(!defined('THINK_PATH')) exit();\r\nreturn \$array = ".var_export($Setting_config, TRUE).";\r\n?>";
			file_put_contents("Data/Conf/config.ini.php",$config);
			$this->success("设置成功");
		}else{
    ...
    {
        $config = D('Systemconfig')->where(array('k'=>'xunsearch'))->find();
		$this->Setting_config = unserialize($config['v']);
		$this->display();

    }

    public function xunsearch_update()
    {
    	if (IS_POST) {
			$post = I('post.');
			$k = 'xunsearch';
    ...
    {
    	$config = D('Systemconfig')->where(array('k'=>'pushbaidu'))->find();
		$this->Setting_config = unserialize($config['v']);
		$this->display();
    }

    public function pushbaidu_update()
    {
    	if (IS_POST) {
			$post = I('post.');
			$k = 'pushbaidu';
    ...
    /*qihoo360推送配置*/
    public function pushqihoo()
    {
    	$config = D('Systemconfig')->where(array('k'=>'pushqihoo'))->find();
		$this->Setting_config = unserialize($config['v']);
		$this->display();
    }

    public function pushqihoo_update()
    {
    	if (IS_POST) {
			$post = I('post.');
			$k = 'pushqihoo';
    ...
    public function tixian_update()
    {
    	if (IS_POST) {
			$post = I('post.');
			if(!preg_match("/^\d*$/",$post['rate'])) {
				$this->error('费率格式不正确');
			}else if((int)$post['rate'] < 1 || (int)$post['rate'] > 100) {
				$this->error('费率取值不正确');
			}
			require "Data/Conf/config.ini.php";
			$Setting_config = $array;
			$Setting_config["rate"] = intval($post['rate']);
			$config = "<?php\r\nif(!defined('THINK_PATH')) exit();\r\nreturn \$array = ".var_export($Setting_config, TRUE).";\r\n?>";
			file_put_contents("Data/Conf/config.ini.php",$config);
			$this->success("设置成功");
		}else{
