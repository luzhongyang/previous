...
var $whitelist;
    ...
    function usercontrol(& $get, & $post) {
        ...
        $this->load("favorite");
         ...
         $this->whitelist="search,spacefollower,vertifyemail,editemail,sendcheckmail";
    }
...
setcookie("useremailcheck","OKadmin",0); //浏览器关闭 是自动失效
    ...
    function onpostrequestmoney(){
      ...
      $user_rmb=$this->user['jine'] ;//$_ENV['userbank']->getmysummoneybytouid($this->user['uid']);
        ...
        if($rmb<1){
        	$this->message("您的可以提现金额小于1块钱，无法提现");
        }
        
        $uid=$this->user['uid'];
         $mod = $this->db->fetch_first("SELECT * FROM " . DB_TABLEPRE . "user_tixian WHERE state=0 and uid='$uid'");
         if($mod!=null){
         	$this->message("您的提现申请正在审核");
         }
         $this->db->query("INSERT INTO " . DB_TABLEPRE . "user_tixian(uid,jine,state,time,beizu) values ('$uid','$rmb','0',{$this->time},'')");
        $this->message("提现申请成功!");
    }
