...
<div style="width:100%; height:15px;color:#000;margin:0px 0px 10px;">
...
</div><? if($message) { ?><table class="table">
    <tr>
    ...
    <table class="table">
        <tbody>
            ...
            <tr>
                <td width="200"  class="altbg2">标题:<input class="txt form-control" name="srchtitle" <? if(isset($srchtitle)) { ?>value="<?=$srchtitle?>" <? } ?>></td>
                <td  width="200" class="altbg2">作者:<input class="txt form-control" name="srchauthor" <? if(isset($srchauthor)) { ?>value="<?=$srchauthor?>" <? } ?>></td>
            ...
            <table  id="table1" class="table"> 
                <tr align="center" class="smalltxt">
                    ...
                    <td width="20%" class="altbg2"><input name="order[]" type="hidden" value="<?=$topic['id']?>"/><strong><?=$topic['title']?></strong></td>
                           ...
                           <td width="10%" class="altbg2" align="center"><? if($topic['isphone']=='0') { ?>否 <? } if($topic['isphone']=='1') { ?><font color=red>是</font>  <? } ?> </td>
                                   <td width="10%" class="altbg2" align="center"><? if($topic['ispc']=='0') { ?>否 <? } if($topic['ispc']=='1') { ?><font color=red>是</font>  <? } ?> </td>
                    <td width="10%" class="altbg2" align="center"><img src="<?=SITE_URL?>static/css/admin/edit.png" style="cursor:pointer" onclick="document.location.href='index.php?admin_topic/edit/<?=$topic['id']?><?=$setting['seo_suffix']?>'"></td>
                </tr>
            </table>
        </li>
