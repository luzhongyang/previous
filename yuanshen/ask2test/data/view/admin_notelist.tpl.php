...
</div><? if(isset($message)) { $type=isset($type)?$type:'correctmsg';  ?><div class="alert  alert-warning"><?=$message?></div><? } ?><form onsubmit="return confirm('该操作不可恢复，您确认要删除这些公告吗？');"  action="index.php?admin_note/remove<?=$setting['seo_suffix']?>"  method=post>
    <table class="table" >
