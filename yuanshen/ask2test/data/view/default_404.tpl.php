...
<!DOCTYPE html>
<html>
 <? global $starttime,$querynum;$mtime = explode(' ', microtime());$runtime=number_format($mtime['1'] + $mtime['0'] - $starttime,6); $setting=$this->setting;$user=$this->user;$headernavlist=$this->nav;$regular=$this->regular;$toolbars="'".str_replace(",", "','", $setting['editor_toolbars'])."'"; ?><head lang="en">
    <meta charset="UTF-8">
...
<body>
    ...
    <a href="<?=SITE_URL?>"><img src="http://www.ask2.cn/data/upload/404_icon.png" ></a>
        <div>
            <h1 >唉呀!</h1>
            <p >你正在寻找的页面无法找到。<a style="color:#ff6600;" href="<?=SITE_URL?>">可能在这里！</a></p>
